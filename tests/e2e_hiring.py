...
with sync_playwright() as p:
 ...
 assert page.evaluate("workerSkillFit(workers[0])")==2.5;assert "SKILL 2.5/5" in page.evaluate("workers[0].status")
 assert page.locator("#cv").get_attribute("data-average-worker-skill")=="2.5"
 assert page.evaluate("JSON.parse(localStorage.getItem(SAVE_KEY)).workers[0].assignment")=="vmc_t2"
 assert page.evaluate("HIRE_ROSTER.candidates.filter(candidate=>candidate.maintenanceQualified).length")==5
 page.evaluate("hireIndex=10;renderHires()")
 assert "Riley Brooks" in page.locator("#hireCard").inner_text();assert page.evaluate("getComputedStyle(document.querySelector('#hireCard .atlasPortrait')).backgroundImage.includes(assetUrl(SPRITES['maintenance-team-profile-atlas-v1']))")
 assert not errors,errors;b.close()
print("PASS: 14-role roster -> five maintenance hires -> skill-qualified persistent NPC behavior")
