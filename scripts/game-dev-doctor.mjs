...
for(const file of ['package.json','pnpm-lock.yaml','wrangler.jsonc','data/release-manifest.json','scripts/build_level_viewer_v4.py','tests/e2e_startup_performance.py'])add(file,fs.existsSync(path.join(root,file))?'pass':'fail',fs.existsSync(path.join(root,file))?'present':'missing');
