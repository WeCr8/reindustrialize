...
#titleShade{position:absolute;inset:0;background:linear-gradient(rgba(3,8,15,.22),rgba(3,8,15,.1) 45%,rgba(3,8,15,.8))}#titleUi{position:relative;width:min(820px,96vw);margin-top:1vh}#companyName,#founderName{font-family:inherit;font-size:20px;text-align:center;text-transform:uppercase;background:rgba(8,11,16,.94);color:#fff;border:3px solid var(--gold);outline:2px solid #000;padding:5px;width:min(420px,90%);margin:4px auto}.founderSetup{margin:4px auto}.avatarChoices{display:grid;grid-template-columns:repeat(5,minmax(78px,1fr));gap:5px;max-width:610px;margin:4px auto 6px}.avatarChoice{display:flex;flex-direction:column;align-items:center;gap:1px;padding:3px 2px;font-size:17px}.avatarChoice canvas{width:40px;height:60px;image-rendering:pixelated;background:#101820;border:1px solid #53606b}.avatarChoice.selected{color:var(--green);border-color:var(--green);background:#0a251b}.avatarChoice.selected canvas{border-color:var(--green)}@media(max-width:600px){.avatarChoices{display:flex;overflow-x:auto;max-width:94vw;justify-content:flex-start}.avatarChoice{min-width:82px}.avatarChoice canvas{width:36px;height:54px}#titleUi{margin-top:1vh}}
...
const state={ toolReady:false, tool:null, stickout:0, toolsSet:[], rawStockReady:false, job:null, materialOrders:[], jobsShipped:0, grades:[],founder:{level:1,xp:0,upgradePoints:0,stats:null} };
const CHAPTER_ONE_TARGET=5;
let phoneSocket=null,phoneRun=false;
let inputMode="auto";const accepts=kind=>inputMode==="auto"||inputMode===kind;
document.getElementById("inputMode").onchange=e=>inputMode=e.target.value;
...
function finishRun(J){
  ...
  const pay=g==="A"?600:g==="B"?450:g==="C"?300:150;
