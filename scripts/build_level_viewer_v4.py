...
const state={ toolReady:false, tool:null, stickout:0, toolsSet:[], rawStockReady:false, job:null, materialOrders:[], jobsShipped:0, grades:[],pendingContract:null,contracts:[],storySequences:[],reputation:CUSTOMER_CONTRACTS.reputationRules.starting,founder:{level:1,xp:0,upgradePoints:0,stats:null} };let paused=false,gameStarted=false,debugOpen=false;
...
document.getElementById("introNext").onclick=()=>{introStep++;if(introStep<introPages.length){renderIntro();return;}document.getElementById("intro").classList.add("closed");const complete=introComplete;introComplete=null;if(complete){complete();return;}if(introSequence==="opening"){mission("meet_zach");say(companyName+" is open.","Zach will tour the floor before your first material order.");setTimeout(()=>startShopTour(true),100);}};
...
document.getElementById("continueGame").disabled=!localStorage.getItem(SAVE_KEY);document.getElementById("continueGame").onclick=restoreGame;document.getElementById("bpause").onclick=openPause;document.getElementById("resumeGame").onclick=closePause;document.getElementById("saveNow").onclick=()=>saveGame("MANUAL");document.getElementById("fullscreenGame").onclick=()=>document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen().catch(()=>{});document.getElementById("returnTitle").onclick=()=>{saveGame("RETURN");location.reload()};
...
setInterval(()=>saveGame("AUTO"),30000);document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="hidden")saveGame("BACKGROUND")});addEventListener("beforeunload",()=>saveGame("EXIT"));
...
function pollGamepad(now){
  ...
  if(gp&&(inputMode==="auto"||inputMode==="gamepad")){
    ...
    if(!paused&&edge(0)){if(!document.getElementById("preFounder").classList.contains("closed"))document.getElementById("preFounderNext").click();else if(!document.getElementById("titleScreen").classList.contains("closed"))document.getElementById("newGame").click();else if(!task.classList.contains("open"))interact();}
