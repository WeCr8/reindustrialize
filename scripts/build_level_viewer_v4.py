...
const STATION_NAMES={planning_desk:"PLANNING DESK",tool_cart:"TOOL CART",vmc_t2:"VMC",nox_terminal:"NOX MATERIAL TERMINAL",lathe_cnc_t2:"CNC LATHE",saw_t1:"BANDSAW",mill_manual_t1:"MANUAL MILL",bench_deburr_t1:"DEBURR BENCH",network_node_t3:"MTCONNECT NODE",handoff_terminal_t4:"HANDOFF TERMINAL",nox_pallet:"RECEIVING",whiteboard:"MISSION BOARD",cobot_t5:"COBOT",amr_t5:"AMR",restroom_station:"EMPLOYEE RESTROOM"};
...
function interact(){
  ...
  if(p.sprite==="lathe_cnc_t2"){trackAnalytics('station_start',{station_id:'lathe_cnc_t2'});openOverlay("CNC LATHE — EQUIPMENT VIEW","Inspect the workholding, stock, turret, tailstock, and control before setup.");showEquipmentView("lathe-open-v1");setTimeout(()=>playSfx("lathe_turret_index"),700);tctrl.innerHTML='<button id="latheClose" class="grn">INSPECTION COMPLETE ▸</button>';document.getElementById("latheClose").onclick=closeOverlay;tz("ZACH: Chuck pressure, stickout, tool clearance. A lathe remembers every shortcut.");return;}
...
function closeOverlay(){task.classList.remove("open");hideRestroomView();hideEquipmentDamage();cancelAnimationFrame(taskAnim);taskAnim=null;clearCycleTimers();stopSfxLoop();machineMotionMode=null;tctrl.innerHTML="";updateGuide();}
function showEquipmentView(id){if(id==='vmc-open-v1'&&maintenanceState().condition<=20){showEquipmentDamage('vmc');return;}hideEquipmentDamage();const im=document.getElementById("tview");im.src=assetUrl(EQUIPMENT_VIEWS[id]);im.style.display="block";tscene.style.display="none";}
...
playZach("zach_cut_stock");
  ...
  document.getElementById("cutStock").onclick=()=>{const v=Number(sl.value);if(Math.abs(v-J.stockLength)>.08){attempts++;tz("ZACH: That blank misses the traveler length. Reset the stop before cutting.");return;}playSfx("bandsaw_cut");state.rawStockReady=true;saveGame('STOCK CUT');trackAnalytics('task_complete',{task_id:'stock_cut',station_id:'saw_t1'});drawSawCut(v,J.stockLength,true);tz("ZACH: Blank cut, labeled, and ready for CNC setup.");setTimeout(()=>{playSfx("bandsaw_power_off");closeOverlay();say("Raw blank ready at "+v.toFixed(2)+' inches.',"Next: gauge the full CNC tool kit.");},700);};}
...
function startAutonomousRun(J){const teamTime=Math.max(-.35,assignedTeamEffect('proveOutTime','vmc_t2')+assignedTeamEffect('setupTime','vmc_t2')),baseDuration=learnerMode?12000:Math.max(8000,20000-(equipmentState().vmc-1)*4000),duration=Math.max(6000,Math.round(baseDuration*(1+teamTime)));state.machineRun={jobId:J.id,status:"running",startedAt:Date.now(),endAt:Date.now()+duration,teamTimeBonus:teamTime};playSfx("cnc_door_close");setTimeout(()=>playSfx("spindle_start"),500);setTimeout(()=>playSfx(J.tool==="twist"?"drill_cut_aluminum":J.tool==="ball"?"ball_mill_cut_aluminum":"end_mill_cut_aluminum",{loop:true}),1400);closeOverlay();say("Verified VMC cycle running autonomously.","Qualified assigned employees improve prove-out and setup time. The station flag shows remaining time; inspection is still required before shipment.");refreshMachineRun();saveGame("AUTO CYCLE");}
