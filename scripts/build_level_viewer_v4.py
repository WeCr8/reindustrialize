...
function assignWorker(h,value){const w=workerFor(h.id);if(!w||(!h.qualifications.includes(value)&&value)||w.assignment===(value||null))return;w.assignment=value||null;w.skillFit=workerSkillFit(w);w.status=w.assignment?'TRAVELING TO '+(STATION_NAMES[w.assignment]||w.assignment.toUpperCase())+' · SKILL '+w.skillFit+'/5':'MEANDERING';saveGame('TEAM ASSIGNMENT');trackAnalytics('worker_assignment',{station_id:analyticsId(w.assignment||'unassigned'),skill_fit:w.skillFit});renderHires();}
...
const state={ toolReady:false, tool:null, stickout:0, toolsSet:[], rawStockReady:false, job:null, materialOrders:[], jobsShipped:0, grades:[],pendingContract:null,contracts:[],storySequences:[],reputation:CUSTOMER_CONTRACTS.reputationRules.starting,founder:{level:1,xp:0,upgradePoints:0,stats:null},machineRun:null,stationRuns:{},equipment:{vmc:1,saw:1,inspection:1},equipmentCooldownUntil:0 };let paused=false,gameStarted=false,debugOpen=false;
...
function stationRun(sprite){if(sprite==='vmc_t2')return state.machineRun;return stationRuns()[sprite]||null;}
function runIsReady(run){return !!run&&(run.status==='ready'||run.status==='inspection');}
...
function stationRunLabel(sprite){const run=stationRun(sprite);if(!run)return'';return runIsReady(run)?'READY TO COLLECT':formatRunTime(run.endAt-Date.now())+' REMAINING';}
...
function completeExpiredRuns(){let changed=false;const saw=stationRuns().saw_t1;if(saw&&saw.status==='running'&&Date.now()>=saw.endAt){saw.status='ready';playSfx("bandsaw_power_off");changed=true;}const vmc=state.machineRun;if(vmc&&vmc.status==='running'&&Date.now()>=vmc.endAt){vmc.status='inspection';machineMotionMode=null;tscene.dataset.motionCode='M30';stopSfxLoop();playSfx("coolant_off");setTimeout(()=>playSfx("spindle_stop"),300);changed=true;}if(changed)saveGame('PRODUCTION COMPLETE');}
function renderProductionHud(){completeExpiredRuns();const root=document.getElementById('productionHud'),active=['saw_t1','vmc_t2'].map(sprite=>[sprite,stationRun(sprite)]).filter(([,run])=>run);root.innerHTML=active.map(([sprite,run])=>{const ready=runIsReady(run),pct=Math.round(runProgress(run)*100);return '<button class="stationTimerFlag '+(ready?'ready':'')+'" id="'+(sprite==='vmc_t2'?'machineFlag':'sawFlag')+'" data-run-station="'+sprite+'"><b>'+(STATION_NAMES[sprite]||sprite)+'</b><br>'+(ready?'✓ COMPLETE · OPEN STATION':formatRunTime(run.endAt-Date.now())+' REMAINING')+'<span class="timerTrack"><i style="width:'+pct+'%"></i></span></button>';}).join('');root.querySelectorAll('[data-run-station]').forEach(button=>button.onclick=()=>walkToStation(button.dataset.runStation,true));const activeTimer=document.getElementById('activeStationTimer'),activeBar=document.querySelector('#activeStationProgress i');if(activeTimer&&task.dataset.station){const run=stationRun(task.dataset.station);if(run){activeTimer.textContent=runIsReady(run)?'COMPLETE · READY TO COLLECT':formatRunTime(run.endAt-Date.now())+' REMAINING';if(activeBar)activeBar.style.width=Math.round(runProgress(run)*100)+'%';}}cv.dataset.activeStationRuns=active.length;cv.dataset.readyStationRuns=active.filter(([,run])=>runIsReady(run)).length;}
...
function interact(preferredSprite=null){
  ...
  if(p.sprite==="saw_t1"){if(!state.job){say("No traveler, no cut length.","Accept a customer job at planning first.");return;}trackAnalytics('station_start',{station_id:'saw_t1'});openSawTask();return;}
  if(p.sprite==="tool_cart"||p.sprite==="presetter_t4"){trackAnalytics('station_start',{station_id:analyticsId(p.sprite)});openToolTask();return;}
  if(p.sprite==="vmc_t2"){
    ...
    trackAnalytics('station_start',{station_id:'vmc_t2'});openVmcTask();return;}
  if(p.sprite==="lathe_cnc_t2"){trackAnalytics('station_start',{station_id:'lathe_cnc_t2'});setTimeout(()=>playSfx("lathe_turret_index"),700);openStationDetails(p);return;}
  ...
  trackAnalytics('station_start',{station_id:analyticsId(p.sprite)});if(p.sprite==='chalkboard')mission('chalkboard');openStationDetails(p);
}
...
function startAutonomousRun(J,fromProof=false){if(state.machineRun){say("The VMC cell is occupied.","Wait for the timer and inspect the part before loading more work. Purchase another VMC to split future production lots.");return;}const capacity=Math.max(1,equipmentState().vmc||1),teamTime=Math.max(-.35,assignedTeamEffect('proveOutTime','vmc_t2')+assignedTeamEffect('setupTime','vmc_t2')),baseDuration=stationDefinition('vmc_t2').cycleMinutes*60000/capacity,duration=Math.max(240000,Math.round(baseDuration*(1+teamTime)));startStationRun('vmc_t2',duration,{jobId:J.id,teamTimeBonus:teamTime,capacity});if(!fromProof){playSfx("cnc_door_close");setTimeout(()=>playSfx("spindle_start"),500);}setTimeout(()=>playSfx(J.tool==="twist"?"drill_cut_aluminum":J.tool==="ball"?"ball_mill_cut_aluminum":"end_mill_cut_aluminum",{loop:true}),fromProof?0:1400);closeOverlay();say("Verified VMC production lot running · "+formatRunTime(duration)+".",capacity+" installed VMC"+(capacity===1?" is":"s are")+" assigned. The cell stays occupied until completion and inspection; added machines split future lot work.");refreshMachineRun();}
...
function openVmcTask(){
  ...
  // Program blanks use large A/B/C/D tabs: quick on mouse, touch, and gamepad.
  ...
  const meanings={o:{"53":"Machine home — not the part zero","54":"Find the part with its saved work offset","90":"Absolute mode — this blank asks where the part is","00":"Rapid move — this blank asks where the part is"},s:{"03":"Spin clockwise for this right-hand cutter","04":"Reverse direction — wrong for this setup","05":"Stop the spindle — no cutting can happen","06":"Change tool — the cutter is already loaded"},c:{"08":"Turn flood coolant on for chip control","09":"Turn coolant off after cutting","30":"End and rewind the program","01":"Optional stop when enabled"}};
  let progHtml='<div class="gcode">';
  for(const ln of J.prog){
    progHtml+=ln.replace(/{{(\w)}}/g,(m,k)=>'<span class="codeBlank" data-code-label="'+k+'">__</span>')+"<br>";
  }
  progHtml+="</div>";
  const codeGroups=Object.entries(choices).map(([key,options],groupIndex)=>'<fieldset class="codeChoiceGroup" data-code-group="'+key+'"><legend>'+(groupIndex+1)+' · '+J.blanks[key].label.toUpperCase()+'</legend><input type="hidden" data-b="'+key+'" value=""><div class="codeTabs">'+options.map((choice,index)=>'<button type="button" class="codeTab" data-code-key="'+key+'" data-code-value="'+choice[0]+'"><b>'+String.fromCharCode(65+index)+'</b><small>'+choice[1].split(' · ')[1]+'</small></button>').join('')+'</div><div class="codeMeaning" data-code-meaning="'+key+'">Choose one tab.</div></fieldset>').join('');
  tctrl.innerHTML=partPreview(J)+progHtml+codeGroups+
    ((guided||learnerMode)?'<button type="button" id="guidedCodeFill" class="quickFillCodes">LOAD ZACH\'S GUIDED SETUP<small>ONE TAP · G54 + M03 + M08</small></button>':'')+
    '<div class="gline"><button id="toggleSetupView">VIEW VMC EQUIPMENT</button><button id="cycst" class="grn">▶ CYCLE START</button>'+(autonomyReady?'<button id="verifiedSetup">LOAD VERIFIED REPEAT SETUP</button><button id="autonomousRun">START AUTO CYCLE & RETURN TO FLOOR</button>':'')+'</div><details open><summary class="ttl">TRUE G & M CODE · READ THE MODAL SEQUENCE</summary><div class="hint"><b>G54</b> selects the stored part origin · <b>M03</b> commands spindle clockwise · <b>M08</b> turns flood coolant on.<br>M06 TOOL CHANGE → G90 ABSOLUTE → G54 WORK OFFSET → G00 RAPID CLEAR → M03 SPINDLE CW → G43 H TOOL LENGTH → M08 COOLANT → G01/G81 CONTROLLED CUT → G80 CANCEL CYCLE → G00 RETRACT → M09 COOLANT OFF → M05 SPINDLE STOP → G28 RETURN → M30 END/REWIND</div></details>';
  const refreshCycleReady=()=>{const fields=[...document.querySelectorAll('#tcontrols [data-b]')],ready=fields.length===3&&fields.every(field=>J.blanks[field.dataset.b].ans.includes(field.value.trim()));const auto=document.getElementById("autonomousRun");if(auto)auto.disabled=!ready;task.dataset.programReady=String(ready);};
  const selectCode=(key,value)=>{const field=document.querySelector('[data-b="'+key+'"]'),group=document.querySelector('[data-code-group="'+key+'"]');field.value=value;group.querySelectorAll('.codeTab').forEach(tab=>{tab.classList.toggle('selected',tab.dataset.codeValue===value);tab.classList.remove('bad');});group.querySelector('[data-code-meaning]').textContent=meanings[key][value];document.querySelector('[data-code-label="'+key+'"]').textContent=value;group.classList.remove('bad');refreshCycleReady();};
  document.querySelectorAll('.codeTab').forEach(tab=>tab.onclick=()=>selectCode(tab.dataset.codeKey,tab.dataset.codeValue));refreshCycleReady();
  ...
  const guidedFill=document.getElementById("guidedCodeFill");if(guidedFill)guidedFill.onclick=()=>{document.querySelectorAll('[data-b]').forEach(field=>selectCode(field.dataset.b,J.blanks[field.dataset.b].ans[0]));guidedFill.disabled=true;guidedFill.innerHTML="✓ GUIDED CODES LOADED<small>G54 FINDS PART · M03 SPINS · M08 COOLS</small>";tz("ZACH: G54 finds the part. M03 spins clockwise. M08 turns coolant on. Now press CYCLE START.");playSfx("terminal_confirm");};
  document.getElementById("cycst").onclick=()=>{
    ...
    document.querySelectorAll('#tcontrols [data-b]').forEach(inp=>{
      ...
      const group=document.querySelector('[data-code-group="'+key+'"]');group.classList.toggle("bad",!ok);if(!ok){group.querySelectorAll('.codeTab.selected').forEach(tab=>tab.classList.add('bad'));allOk=false;}
    });
  ...
  const verified=document.getElementById("verifiedSetup");if(verified)verified.onclick=()=>{document.querySelectorAll('[data-b]').forEach(inp=>selectCode(inp.dataset.b,J.blanks[inp.dataset.b].ans[0]));tz("ZACH: This repeat setup earned assisted loading through three good jobs. You still verify stock, workholding, offsets, tools, and first article.");document.getElementById("cycst").click();};
