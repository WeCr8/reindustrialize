...
station_operations = json.load(open(os.path.join(ROOT, "data", "station-operations.json")))
...
#bvoice,#audioState{position:fixed;z-index:150;top:10px}#bvoice{right:10px}#audioState{right:10px;top:52px;background:rgba(5,8,12,.94);border:1px solid #3fd08a;padding:3px 8px;color:var(--green)}
...
#productionHud{position:absolute;right:8px;top:8px;z-index:11;display:grid;gap:5px;max-width:min(300px,55%);max-height:58%;overflow-y:auto;line-height:1.1}#productionHud:empty{display:none}.stationTimerFlag{padding:7px 9px;background:rgba(5,10,15,.96);border:3px solid var(--gold);color:#fff;text-align:left;box-shadow:2px 2px 0 #000}.stationTimerFlag.ready{border-color:var(--green);color:var(--green);animation:timerReady .8s steps(2,end) infinite}.timerTrack{height:7px;margin-top:4px;background:#27313a;border:1px solid #000}.timerTrack i{display:block;height:100%;background:var(--gold)}.stationTimerFlag.ready .timerTrack i{background:var(--green)}@keyframes timerReady{50%{filter:brightness(1.35)}}
.stationDetailGrid{display:grid;grid-template-columns:minmax(160px,38%) 1fr;gap:12px;align-items:start}.stationDetailGrid img{width:100%;max-height:300px;object-fit:contain;image-rendering:pixelated;background:#071019;border:3px solid var(--gold)}.stationStatusCard{padding:9px;border:2px solid #46525d;background:#09121a}.stationProgress{height:18px;background:#202a31;border:2px solid #000;margin:8px 0}.stationProgress i{display:block;height:100%;background:linear-gradient(90deg,var(--gold),var(--green));width:0}.stationMission{color:#fff;font-size:20px}.stationMode{display:inline-block;padding:3px 7px;border:1px solid var(--sky);color:var(--sky);margin-bottom:6px}
.partPreview{display:grid;grid-template-columns:minmax(190px,42%) 1fr;gap:10px;align-items:center;margin:7px 0;padding:8px;background:#071019;border:2px solid var(--gold)}.partPreview svg{width:100%;height:auto;max-height:190px;background:#111a22;border:1px solid #46525d}.partPreview b{color:var(--gold);font-size:22px}.partPreview p{margin:4px 0;color:#fff}.partPreview .featureList{color:#9dd6ff;font-size:17px}.quickFillCodes{width:100%;min-height:48px;margin:7px 0;border-color:var(--green);color:#07140f;background:var(--green);font-size:21px}.quickFillCodes small{display:block;font-size:14px;color:#0b3827}
.codeChoiceGroup{margin:7px 0;padding:7px;border:2px solid #46525d;background:#09121a}.codeChoiceGroup legend{color:#fff;font-size:19px}.codeTabs{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.codeTab{min-height:46px;padding:4px;color:#fff;border-color:#65717c}.codeTab b{display:block;color:var(--gold);font-size:21px}.codeTab small{display:block;font-size:13px;line-height:1}.codeTab.selected{background:#173a2d;border-color:var(--green);color:var(--green)}.codeTab.bad{background:#3d1918;border-color:var(--alert)}.codeMeaning{min-height:22px;margin-top:5px;color:#9dd6ff}
...
#missions button{display:block;width:100%;font:17px VT323,monospace;text-align:left;margin:3px 0;padding:4px 7px;color:#c4c9d0;background:#0b1016;border:1px solid #39424e;box-shadow:none;}
#missions button:hover,#missions button:focus-visible,#missions button.selected{color:#fff;border-color:var(--gold);background:#18202a;outline:none;}
#missions button.done{color:var(--green);border-color:#245d47;}
#missions button.done::before{content:'☑ ';} #missions button::before{content:'☐ ';}
...
@media (max-width: 900px), (pointer:coarse) and (max-width: 1100px){
  ...
  #sideR{display:block;width:min(100%,720px);order:2}#sideR>.panel:first-child{display:none}#sideR>.panel:nth-child(2){margin-top:0!important;padding:6px 8px}#sideR>.panel:nth-child(2)>.ttl,#sideR>.panel:nth-child(2)>div[style],#chapterProgress,#qbar,#qpct{display:none}#missions{display:flex;gap:4px;overflow-x:auto;margin-bottom:5px}#missions button{flex:0 0 auto;width:auto;max-width:170px;font-size:15px;white-space:nowrap;padding:3px 6px}#objectiveGuide{margin-top:0}#objectiveHow{font-size:17px}#objectiveAction{min-height:44px}
  ...
  .hint{display:none;} #bar button{font-size:15px;padding:2px 8px;}#bvoice,#audioState{position:static}#audioState{display:none}
}
...
@media(max-width:700px){#titleScreen:not(.closed)~#bvoice,#titleScreen:not(.closed)~#audioState{display:none}}
...
/* The final Zach workshop scene is the one founder-selection screen. */
#preFounder.selecting{place-items:center}#preFounder.selecting #preFounderCopy{display:none}#preFounder.selecting #preFounderCard{background-position:center;background-size:cover}
#preFounder.selecting #preFounderArt{position:absolute;inset:0}#preFounder.selecting #titleUi{position:absolute;z-index:3;top:1.5%;right:1.5%;bottom:1.5%;width:min(850px,67%);max-height:none;margin:0;padding:10px 10px 92px;overflow:auto;background:linear-gradient(90deg,rgba(5,8,12,.84),rgba(5,8,12,.97));border:3px solid var(--gold);box-shadow:8px 8px 0 #000}
#preFounder.selecting #gameLogo{font-size:clamp(30px,4.2vw,58px)}#preFounder.selecting #tagline{font-size:clamp(15px,1.6vw,21px);margin-bottom:8px}#preFounder.selecting .avatarChoices{max-width:100%}#preFounder.selecting #founderProfile{max-width:100%;grid-template-columns:150px 1fr;min-height:220px}#preFounder.selecting #founderProfile .atlasPortrait{width:150px;height:200px}
#preFounder.selecting .titleMenu{position:static;left:auto;bottom:auto;transform:none;width:min(760px,100%);margin:8px auto 0;grid-template-columns:2fr 1fr 1.5fr 1fr}
@media(max-width:700px){#preFounder.selecting .titleMenu{grid-template-columns:2fr 1fr}}
@media(max-width:900px){#preFounder.selecting #titleUi{left:1%;right:1%;top:1%;bottom:1%;width:98%;padding-bottom:118px;background:rgba(5,8,12,.94)}#preFounder.selecting #gameLogo{font-size:34px}#preFounder.selecting #founderProfile{grid-template-columns:108px 1fr}#preFounder.selecting #founderProfile .atlasPortrait{width:108px;height:144px}}
...
@media(max-width:900px){
  ...
  #productionHud{right:5px;top:5px;max-width:56%;gap:3px}.stationTimerFlag{padding:4px 6px;font-size:13px;border-width:2px}.timerTrack{height:5px}.stationDetailGrid{grid-template-columns:1fr}.stationDetailGrid img{max-height:180px}
...
<div id="layout">
  ...
  <div id="cwrap"><canvas id="cv"></canvas><div id="productionHud" aria-live="polite"></div><div id="stationPrompt"><span id="stationPromptText"></span><button id="stationOpen">USE / OPEN</button></div></div>
  <div id="sideR">
    <div class="panel">
    ...
    <div class="panel" style="margin-top:10px">
      ...
      <div id="missions">
        <button type="button" data-m="meet_zach">MEET ZACH AT THE BAY</button>
        <button type="button" data-mission-action="customers">ANSWER PHONE / JOBLINE</button>
        <button type="button" data-m="planning_desk">ACCEPT A CUSTOMER JOB</button>
        <button type="button" data-m="task_tool">SET THE CNC TOOL KIT ★</button>
        <button type="button" data-m="task_vmc">RUN & INSPECT THE VMC ★</button>
      </div>
...
const STATION_OPERATIONS=__STATION_OPERATIONS__;
...
function enterFounderSelection(){if(zachAudio)zachAudio.pause();const pre=document.getElementById("preFounder"),ui=document.getElementById("titleUi");pre.classList.add("selecting");document.getElementById("titleScreen").classList.add("closed");document.getElementById("preFounderCard").append(ui);ui.scrollTop=0;renderFounderPreview();document.querySelector(".avatarChoice.selected")?.focus();}
document.getElementById("preFounderNext").onclick=()=>{if(!preFounderStarted){preFounderStarted=true;renderPreFounder(true);return;}if(preFounderStep===preFounderBeats.length-1){enterFounderSelection();return;}preFounderStep++;renderPreFounder(true);};renderPreFounder(false);
const IMG={};
...
const analyticsId=value=>String(value||'unknown').toLowerCase().replace(/[^a-z0-9_-]/g,'_').slice(0,48);
function trackAnalytics(event,params={}){window.dispatchEvent(new CustomEvent('reind:analytics',{detail:{event,params}}));}
...
function atlasStyle(name,cell,columns=5,rows=2){const x=cell%columns,y=Math.floor(cell/columns),px=columns===1?0:x*100/(columns-1),py=rows===1?0:y*100/(rows-1);return 'background-image:url('+assetUrl(SPRITES[name])+');background-size:'+(columns*100)+'% '+(rows*100)+'%;background-position:'+px+'% '+py+'%;'}
function candidateAtlasName(candidate,type='profile'){if(type==='sprite'&&candidate.maintenanceQualified&&candidate.spriteAtlas)return HIRE_ROSTER.maintenanceSpriteAtlas||candidate.spriteAtlas;return type==='profile'?(candidate.profileAtlas||HIRE_ROSTER.profileAtlas):(candidate.spriteAtlas||HIRE_ROSTER.spriteAtlas);}
function candidateAtlasStyle(candidate,type='profile'){return atlasStyle(candidateAtlasName(candidate,type),candidate.atlasCell,candidate.atlasColumns||5,candidate.atlasRows||2);}
...
function selectFounderButton(b){const changed=selectedAvatar!==b.dataset.avatar;selectedAvatar=b.dataset.avatar;document.querySelectorAll(".avatarChoice").forEach(x=>x.classList.toggle("selected",x===b));renderFounderPreview();setTitleArt();setStoryArt("opening");b.scrollIntoView({block:"nearest",inline:"center"});if(changed)trackAnalytics('founder_select',{archetype:analyticsId(selectedAvatar)});}
...
const STATION_SKILLS={saw_t1:['problem_solving'],bench_deburr_t1:['problem_solving'],mill_manual_t1:['cnc_programming','problem_solving'],vmc_t2:['cnc_programming','problem_solving'],lathe_cnc_t2:['cnc_programming','problem_solving'],planning_desk:['communication','software_dev'],whiteboard:['communication'],handoff_terminal_t4:['communication','software_dev'],network_node_t3:['software_dev','automation'],cobot_t5:['automation','problem_solving'],amr_t5:['automation','software_dev'],nox_terminal:['communication','software_dev'],nox_pallet:['communication','problem_solving'],tool_cart:['problem_solving']};
function workerSkillFit(worker,station=worker?.assignment){if(!worker?.candidate||!station)return 0;const keys=STATION_SKILLS[station]||['problem_solving','communication'];return Math.round(keys.reduce((sum,key)=>sum+(worker.candidate.skills[key]||0),0)/keys.length*10)/10;}
function assignedTeamEffect(key,station){return workers.filter(worker=>worker.assignment===station).reduce((sum,worker)=>sum+Number(worker.candidate?.teamEffects?.[key]||0),0);}
...
function workerHasCommittedWork(workerId){return Object.values(workerQueues()).flat().some(item=>item.workerId===workerId&&item.status!=='complete');}
function assignWorker(h,value){const w=workerFor(h.id);if(!w||(!h.qualifications.includes(value)&&value)||w.assignment===(value||null))return;if(workerHasCommittedWork(w.id)){say(h.name+' is committed to queued production.','Finish or collect the assigned batches before moving this worker to another station.');return;}w.assignment=value||null;w.skillFit=workerSkillFit(w);w.status=w.assignment?'TRAVELING TO '+(STATION_NAMES[w.assignment]||w.assignment.toUpperCase())+' · SKILL '+w.skillFit+'/5':'MEANDERING';saveGame('TEAM ASSIGNMENT');trackAnalytics('worker_assignment',{station_id:analyticsId(w.assignment||'unassigned'),skill_fit:w.skillFit});renderHires();}
function hireCandidate(h){if(hired.has(h.id))return;if(coins<h.hireCost)return alert("Not enough coins.");const returnToProfile=document.getElementById("profile").classList.contains("open");coins-=h.hireCost;hired.add(h.id);workers.push({id:h.id,candidate:h,x:P.x+1,y:P.y,assignment:null,status:'MEANDERING',skillFit:0,nextMove:0,workPulse:0});addCoins(0);saveGame('TEAM HIRE');trackAnalytics('hire_role',{role_id:analyticsId(h.title)});renderHires();if(!storySeen("first_hire")){document.getElementById("hire").classList.remove("open");setTimeout(()=>showStorySequence("first_hire",()=>{document.getElementById("hire").classList.add("open");if(returnToProfile)showProfileV2(h);else renderHires();}),100);}}
...
document.getElementById("hirePrev").onclick=()=>{hireIndex=(hireIndex-1+HIRE_ROSTER.candidates.length)%HIRE_ROSTER.candidates.length;renderHires();};document.getElementById("hireNext").onclick=()=>{hireIndex=(hireIndex+1)%HIRE_ROSTER.candidates.length;renderHires();};document.getElementById("bteam").onclick=()=>{loadSprite(HIRE_ROSTER.spriteAtlas);loadSprite(HIRE_ROSTER.profileAtlas);HIRE_ROSTER.candidates.forEach(candidate=>{loadSprite(candidateAtlasName(candidate,'sprite'));loadSprite(candidateAtlasName(candidate,'profile'))});hireIndex=0;renderHires();document.getElementById("hire").classList.add("open");};document.getElementById("hireClose").onclick=()=>document.getElementById("hire").classList.remove("open");
...
const state={ toolReady:false, tool:null, stickout:0, toolsSet:[], rawStockReady:false, job:null, materialOrders:[], jobsShipped:0, grades:[],pendingContract:null,contracts:[],storySequences:[],reputation:CUSTOMER_CONTRACTS.reputationRules.starting,founder:{level:1,xp:0,upgradePoints:0,stats:null},machineRun:null,stationRuns:{},workerQueues:{},workerWip:{sawBlanks:0,finishedParts:0},equipment:{vmc:1,lathe:1,saw:1,inspection:1},equipmentCooldownUntil:0 };let paused=false,gameStarted=false,debugOpen=false;
function stationRuns(){return state.stationRuns||(state.stationRuns={});}
function workerQueues(){return state.workerQueues||(state.workerQueues={});}
function stationWorkerQueue(sprite){return workerQueues()[sprite]||(workerQueues()[sprite]=[]);}
...
function equipmentPrice(item){const installed=equipmentState()[item.id]||0,purchased=Math.max(0,installed-(item.included||0));return item.cost*(purchased+1);}
function equipmentNeedsMaintenance(item){return ['surface_grinder','cnc_lathe','polymer_print_farm','five_axis_vmc','robotic_tig','metal_additive','cmm','xray','cobot'].includes(item.id);}
function currentFacilityChapter(){return map?.id==='bay_02'?2:1;}
function workerFillsRole(role){return workers.some(worker=>role==='maintenance_technician'?worker.candidate?.maintenanceQualified:worker.candidate?.role===role);}
function missingEquipmentRoles(item){return (item.requiredRoleIds||[]).filter(role=>!workerFillsRole(role));}
function roboticsSpaceUsed(){const owned=equipmentState();return EQUIPMENT_MARKET.items.reduce((sum,item)=>sum+(owned[item.id]||0)*(item.installSpaceSqFt||0),0);}
function equipmentPurchaseBlock(item){const owned=equipmentState(),price=equipmentPrice(item),missingRoles=missingEquipmentRoles(item),facility=FACILITIES[currentFacilityChapter()-1],spaceBudget=facility.robotLogisticsSpaceSqFt||0;if(state.jobsShipped<item.unlockJobs)return 'SHIP '+item.unlockJobs+' JOBS FIRST';if(currentFacilityChapter()<(item.minimumFacilityChapter||item.chapter))return 'MOVE TO '+item.facility.toUpperCase()+' FIRST';if(item.maxOwned&&(owned[item.id]||0)>=item.maxOwned)return 'OWNERSHIP CAP REACHED · '+item.maxOwned;if(workers.length<item.requiredHeadcount)return 'HIRE '+(item.requiredHeadcount-workers.length)+' MORE EMPLOYEE'+(item.requiredHeadcount-workers.length===1?'':'S');if(missingRoles.length)return 'MISSING QUALIFIED ROLES · '+missingRoles.map(role=>role.replaceAll('_',' ').toUpperCase()).join(' · ');if((item.requiresMaintenance||equipmentNeedsMaintenance(item))&&!maintenanceWorker())return 'HIRE A QUALIFIED MAINTENANCE TECHNICIAN';if(item.installSpaceSqFt&&roboticsSpaceUsed()+item.installSpaceSqFt>spaceBudget)return 'NEED MORE ROBOT LOGISTICS SPACE';if(coins<price)return 'NEED '+(price-coins)+' MORE COINS';return '';}
function secureEquipmentPurchase(item){const price=equipmentPrice(item),block=equipmentPurchaseBlock(item);if(block){playSfx('terminal_error');openEquipmentMarket();return false;}coins-=price;const owned=equipmentState();owned[item.id]=(owned[item.id]||0)+1;addCoins(0);saveGame('EQUIPMENT PURCHASE');trackAnalytics('equipment_purchase',{equipment_id:analyticsId(item.id),tier:item.chapter});playSfx('terminal_confirm');openEquipmentMarket();return true;}
function renderEquipmentStaffing(){EQUIPMENT_MARKET.items.forEach(item=>{const card=document.querySelector('[data-equipment="'+item.id+'"]');if(!card)return;const missingRoles=missingEquipmentRoles(item),details=document.createElement('div'),block=equipmentPurchaseBlock(item);details.className='staffingRequirement';details.innerHTML='<b>STAFF GATE:</b> '+workers.length+'/'+item.requiredHeadcount+' EMPLOYEES · '+item.requiredRoles.join(' · ')+(item.maxOwned?' · CAP '+item.maxOwned:'')+(item.installSpaceSqFt?' · '+item.installSpaceSqFt+' SQ FT LOGISTICS SPACE':'')+(missingRoles.length?'<br><span class="hint">ROLE FIT IS CHECKED FROM EACH HIREE PROFILE.</span>':'');card.querySelector('div:last-child').append(details);const button=card.querySelector('[data-buy-equipment]');if(button){const price=equipmentPrice(item);button.dataset.price=String(price);button.textContent='BUY FOR '+price+' COINS';button.disabled=!!block;button.onclick=()=>secureEquipmentPurchase(item);if(block){const warning=document.createElement('div');warning.className='hint staffingLock';warning.textContent=block;button.after(warning);}}});}
...
function renderEquipmentArt(){EQUIPMENT_MARKET.items.forEach(item=>{const card=document.querySelector('[data-equipment="'+item.id+'"]'),art=card?.querySelector('.equipmentProductArt');if(art)art.style.cssText=marketAtlasStyle(item);if(card&&item.kidSummary&&!card.querySelector('.robotKidSummary')){const summary=document.createElement('p');summary.className='kidWord robotKidSummary';summary.textContent=item.kidSummary;card.querySelector('.equipmentMeta').after(summary);}});}
function renderMaintenanceLabels(){EQUIPMENT_MARKET.items.forEach(item=>{if(!(item.requiresMaintenance||equipmentNeedsMaintenance(item)))return;const card=document.querySelector('[data-equipment="'+item.id+'"]');if(card&&!card.querySelector('.maintenanceNeed')){const label=document.createElement('div');label.className='hint maintenanceNeed';label.textContent='MAINTENANCE TECHNICIAN REQUIRED';card.querySelector('.staffingRequirement')?.after(label);}});}
let robotPreviewTimer=null,robotPreviewFrame=0;
function openRobotProfile(profile){let modal=document.getElementById('robotProfile');if(!modal){modal=document.createElement('div');modal.id='robotProfile';modal.style.cssText='position:fixed;inset:0;z-index:190;background:#05070af2;display:none;place-items:center;padding:12px';modal.innerHTML='<div class="panel" style="width:min(760px,96vw);max-height:94dvh;overflow:auto"><div id="robotProfileBody"></div><button id="robotProfileClose">RETURN TO STORE</button></div>';document.body.append(modal);document.getElementById('robotProfileClose').onclick=()=>modal.style.display='none';}const stats=Object.entries(profile.stats).map(([key,value])=>'<div class="skillRow"><span>'+key.replaceAll('_',' ').toUpperCase()+'</span><span>'+"■".repeat(value)+"□".repeat(5-value)+'</span></div>').join(''),image=profile.profileImage?'<img style="width:100%;max-height:520px;object-fit:contain;image-rendering:pixelated" src="'+assetUrl(EQUIPMENT_VIEWS[profile.profileImage])+'" alt="'+profile.displayName+' profile">':'';document.getElementById('robotProfileBody').innerHTML='<div class="ttl">ROBOTICS PLATFORM PROFILE · 8 FPS PREVIEW</div><h2>'+profile.displayName+'</h2>'+image+stats+'<p>'+((profile.approvedTasks||[profile.role]).join(' · '))+'</p>'+(profile.prohibitedTasks?'<p class="hint"><b>NOT APPROVED:</b> '+profile.prohibitedTasks.join(' · ')+'</p>':'');modal.style.display='grid';}
function renderRobotPreviews(){clearInterval(robotPreviewTimer);const profiles=ROBOTICS_PROFILES.platforms;profiles.forEach(profile=>{const card=document.querySelector('[data-equipment="'+profile.equipment+'"]');if(!card)return;const preview=document.createElement('div');preview.className='robotSpritePreview';preview.dataset.robot=profile.equipment;preview.style.cssText='height:150px;background-repeat:no-repeat;background-color:#071019;border:2px solid var(--green);image-rendering:pixelated;margin:5px 0';card.querySelector('div:last-child').prepend(preview);const button=document.createElement('button');button.textContent='VIEW '+profile.displayName.toUpperCase()+' PROFILE';button.onclick=()=>openRobotProfile(profile);preview.after(button);});const draw=()=>{profiles.forEach(profile=>{const preview=document.querySelector('[data-robot="'+profile.equipment+'"]');if(!preview)return;const frame=profile.frames[robotPreviewFrame%profile.frames.length],x=frame%profile.columns,y=Math.floor(frame/profile.columns),px=profile.columns===1?0:x*100/(profile.columns-1),py=profile.rows===1?0:y*100/(profile.rows-1);preview.style.backgroundImage='url('+assetUrl(SPRITES[profile.spriteAtlas])+')';preview.style.backgroundSize=(profile.columns*100)+'% '+(profile.rows*100)+'%';preview.style.backgroundPosition=px+'% '+py+'%';});robotPreviewFrame++;};draw();robotPreviewTimer=setInterval(draw,125);}
function openOperatingProfile(item,profile){let modal=document.getElementById('operatingProfile');if(!modal){modal=document.createElement('div');modal.id='operatingProfile';modal.style.cssText='position:fixed;inset:0;z-index:191;background:#05070af2;display:none;place-items:center;padding:12px';modal.innerHTML='<div class="panel" style="width:min(820px,96vw);max-height:94dvh;overflow:auto"><div id="operatingProfileBody"></div><button id="operatingProfileClose">RETURN TO STORE</button></div>';document.body.append(modal);document.getElementById('operatingProfileClose').onclick=()=>modal.style.display='none';}const stats=Object.entries(profile.stats).map(([key,value])=>'<div class="skillRow"><span>'+key.replaceAll('_',' ').toUpperCase()+'</span><span>'+"■".repeat(value)+"□".repeat(5-value)+'</span></div>').join('');document.getElementById('operatingProfileBody').innerHTML='<div class="ttl">EQUIPMENT OPERATING PROFILE · '+item.category.toUpperCase()+'</div><h2>'+item.name+'</h2><div class="equipmentProductArt" style="height:320px;'+marketAtlasStyle(item)+'"></div><div class="panel"><b>REAL SETUP:</b> '+profile.realSetup+'<br><b>REAL CYCLE:</b> '+profile.realCycle+'<br><b>GAME CYCLE:</b> '+profile.gameCycleSeconds[0]+'–'+profile.gameCycleSeconds[1]+' SECONDS<br><b>CAPACITY:</b> '+profile.capacity+'</div>'+stats+'<p><b>NEEDS:</b> '+profile.dependencies.join(' · ')+'</p><p class="hint">'+EQUIPMENT_OPERATING_PROFILES.timeRule+'</p>';modal.style.display='grid';}
function renderOperatingProfiles(){EQUIPMENT_OPERATING_PROFILES.profiles.forEach(profile=>{const item=EQUIPMENT_MARKET.items.find(entry=>entry.id===profile.equipment),card=document.querySelector('[data-equipment="'+profile.equipment+'"]');if(!item||!card)return;const button=document.createElement('button');button.className='operatingProfileButton';button.textContent='VIEW STATS, CAPACITY & CYCLE TIME';button.onclick=()=>openOperatingProfile(item,profile);card.querySelector('div:last-child').append(button);});}
const openEquipmentMarketBase=openEquipmentMarket;openEquipmentMarket=function(){openEquipmentMarketBase();if(!document.getElementById('storeCoach')){const coach=document.createElement('section');coach.id='storeCoach';coach.className='panel';coach.setAttribute('aria-live','polite');document.getElementById('equipmentMarketBody').before(coach)}renderStoreCoach();renderEquipmentArt();renderEquipmentStaffing();renderMaintenanceLabels();renderRobotPreviews();renderOperatingProfiles();};
...
function selectControl(mode){const changed=inputMode!==mode;inputMode=mode;localStorage.setItem("reindustrialize.inputMode",mode);renderControlSelection();if(changed)trackAnalytics('control_mode',{mode:analyticsId(mode)});if(mode==="phone"){document.getElementById("connect").classList.add("open");if(!document.getElementById("pairQr").getAttribute("src"))createPhoneSession();}}
...
document.getElementById("introNext").onclick=()=>{introStep++;if(introStep<introPages.length){renderIntro();return;}document.getElementById("intro").classList.add("closed");const complete=introComplete;introComplete=null;if(complete){complete();return;}if(introSequence==="opening"){mission("meet_zach");say(companyName+" is open.","Answer the first customer call. Zach teaches each station when you reach it; the full Shop Tour remains optional and replayable.","zach_response_next_step");updateGuide();}};
...
function enterJobShop(){setMap("bay_02");sel("b2");trackAnalytics('chapter_milestone',{chapter:2});trackAnalytics('facility_milestone',{facility_id:'job_shop'});say(companyName+" has entered the Job Shop chapter.","Hire a team, control flow, and earn the next facility.");}
renderIntro();
...
function partPreview(J){
  const common='<defs><linearGradient id="metal" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e3e8ec"/><stop offset=".45" stop-color="#8896a3"/><stop offset="1" stop-color="#4b5966"/></linearGradient></defs>';
  let drawing='',features='';
  if(J.anim==='drill'){
    drawing='<rect x="38" y="34" width="244" height="132" rx="4" fill="url(#metal)" stroke="#05080c" stroke-width="5"/><path d="M38 55h244M58 34v132" stroke="#f3f7fa" opacity=".45"/>'+[[88,76],[162,76],[88,126],[162,126]].map(([x,y])=>'<circle cx="'+x+'" cy="'+y+'" r="12" fill="#111a22" stroke="#e8b93b" stroke-width="4"/>').join('')+'<path d="M205 76h58M205 126h58" stroke="#4a9fd4" stroke-width="2" stroke-dasharray="5 4"/><text x="209" y="70" fill="#fff" font-size="13">Ã˜0.257 THRU</text><text x="209" y="121" fill="#fff" font-size="13">0.55 DEEP</text>';
    features='RECTANGULAR 6061-T6 PLATE · FOUR Ã˜0.257 HOLES · 0.55 IN DEEP';
  }else if(J.anim==='pocket'){
    drawing='<path d="M42 55l210-20 48 30-210 24z" fill="#d9e0e5" stroke="#05080c" stroke-width="4"/><path d="M42 55v88l48 30V89M90 89l210-24v85L90 173" fill="url(#metal)" stroke="#05080c" stroke-width="4"/><path d="M112 91l126-13 30 18-126 16z" fill="#202b34" stroke="#e8b93b" stroke-width="4"/><path d="M142 112v35l126-14V96" fill="#3f4d58" stroke="#111"/><text x="116" y="70" fill="#fff" font-size="13">2.0 Ã— 1.4 POCKET</text><text x="172" y="160" fill="#fff" font-size="13">0.375 DEEP</text>';
    features='RECTANGULAR 6061-T6 BLOCK · FLAT-BOTTOM POCKET · STRAIGHT WALLS';
  }else{
    drawing='<path d="M42 137h256v43H42z" fill="url(#metal)" stroke="#05080c" stroke-width="4"/><path d="M90 137c20-79 140-79 160 0z" fill="#aeb9c2" stroke="#e8b93b" stroke-width="4"/><path d="M170 47v90M164 53h12M164 131h12" stroke="#4a9fd4" stroke-width="2"/><text x="181" y="91" fill="#fff" font-size="13">R0.75</text><path d="M90 137c20-79 140-79 160 0" fill="none" stroke="#fff" stroke-dasharray="5 4"/>';
    features='6061-T6 BLOCK · SMOOTH CONVEX DOME · R0.75 FINISHED SURFACE';
  }
  return '<section class="partPreview" data-part-shape="'+J.anim+'"><svg viewBox="0 0 340 205" role="img" aria-label="Finished '+J.anim+' part shape">'+common+drawing+'</svg><div><b>WHAT YOU ARE MAKING</b><p>'+J.card+'</p><div class="featureList">'+features+'</div><p>RAW BLANK '+J.stockLength.toFixed(2)+' IN LONG â†’ CUT FEATURES â†’ INSPECTED PART</p></div></section>';
}
let jobIdx=0;
...
function validateSaveSnapshot(save){
  ...
  const workerById=new Map();for(const worker of save.workers){const candidate=HIRE_ROSTER.candidates.find(entry=>entry.id===worker.id);if(!candidate)fail("unknown worker");if(worker.assignment&&(!candidate.qualifications.includes(worker.assignment)||!(worker.assignment in STATION_OPERATIONS.stations)))fail("invalid worker assignment");workerById.set(worker.id,{worker,candidate});}
  ...
  if(s.workerQueues===undefined)s.workerQueues={};if(s.workerWip===undefined)s.workerWip={sawBlanks:0,finishedParts:0};if(s.equipment&&s.equipment.lathe===undefined)s.equipment.lathe=1;
  if(!s.workerQueues||typeof s.workerQueues!=="object"||Array.isArray(s.workerQueues))fail("invalid worker queues");
  for(const [station,queue] of Object.entries(s.workerQueues)){if(!Array.isArray(queue))fail("invalid queue station");if(!['saw_t1','vmc_t2','lathe_cnc_t2'].includes(station)){if(queue.length===0){delete s.workerQueues[station];continue;}fail("invalid queue station");}const slots=station==='vmc_t2'?Math.max(1,Number(s.equipment?.vmc||1)):station==='lathe_cnc_t2'?Math.max(1,Number(s.equipment?.lathe||1)):Math.max(1,Number(s.equipment?.saw||1));if(queue.length>slots*3)fail("queue exceeds installed capacity");for(const item of queue){const worker=workerById.get(item?.workerId),duration=stationDurationMs(station);if(!item||item.station!==station||item.jobId!==s.job?.id||!['queued','running','complete'].includes(item.status)||!Number.isFinite(item.startedAt)||!Number.isFinite(item.endAt)||item.endAt<=item.startedAt||item.durationMs!==duration||!Number.isInteger(item.slot)||item.slot<0||item.slot>=slots||!Number.isFinite(item.cost)||item.cost<0||!Number.isFinite(item.reward)||item.reward<0||item.reward>1000||!worker||worker.worker.assignment!==station||!worker.candidate.qualifications.includes(station))fail("invalid queued work");}}
  if(!s.workerWip||!Number.isSafeInteger(s.workerWip.sawBlanks)||s.workerWip.sawBlanks<0||!Number.isSafeInteger(s.workerWip.finishedParts)||s.workerWip.finishedParts<0)fail("invalid worker WIP");
  return save;
}
...
function mission(id){
  const el=document.querySelector('#missions [data-m="'+id+'"]');
...
const MISSION_ROUTES={meet_zach:"mentor",planning_desk:"planning_desk",task_tool:"tool_cart",task_vmc:"vmc_t2"};
function selectMission(id){
  document.querySelectorAll("#missions [data-m]").forEach(el=>el.classList.toggle("selected",el.dataset.m===id));
  if(!gameStarted){say("Launch your company to begin this mission.","Choose a founder, name the founder and factory, then select LAUNCH COMPANY.");return;}
  if(id==="meet_zach"){openMentor();return;}
  const requested=MISSION_ROUTES[id],objective=currentObjective();
  if(id==="planning_desk"&&objective?.action==="customers"){say("A customer starts every real job.","Answer the shop phone or open JobLine, review the request, and accept a contract before Planning releases work.");openCustomers("shop_phone");return;}
  if(id==="planning_desk"&&!state.materialOrders.length){say("First, order certified raw material.","This mission is selected. I am routing you to NOX Materials before Planning can release a traveler.");walkToStation("nox_terminal",true);return;}
  if(id==="task_tool"&&(!state.job||!state.rawStockReady)){say("Tool setup needs a released job and a cut blank.","I am routing you to the next required production step so the tool setup will activate correctly.");if(objective)walkToStation(objective.sprite,true);return;}
  if(id==="task_vmc"&&(!state.job||!state.rawStockReady||!state.toolReady)){say("The VMC needs a job, cut blank, and three gauged tools.","I am routing you to the next unfinished prerequisite. The VMC will unlock as soon as the setup is safe.");if(objective)walkToStation(objective.sprite,true);return;}
  walkToStation(requested,true);
}
document.querySelectorAll("#missions [data-m]").forEach(el=>el.onclick=()=>selectMission(el.dataset.m));
document.querySelector('[data-mission-action="customers"]').onclick=()=>{document.querySelectorAll("#missions button").forEach(el=>el.classList.toggle("selected",el.dataset.missionAction==="customers"));if(!gameStarted){say("Launch your company to answer customer work.","Name the founder and factory first.");return;}openCustomers("shop_phone");};
...
function drawWorkers(){for(const w of workers){const atlasName=candidateAtlasName(w.candidate,'sprite'),im=IMG[atlasName];if(!im?.complete)continue;const columns=w.candidate.atlasColumns||5,rows=w.candidate.atlasRows||2,sw=im.naturalWidth/columns,sh=im.naturalHeight/rows,cell=w.candidate.atlasCell,sx=(cell%columns)*sw,sy=Math.floor(cell/columns)*sh,bob=w.status.startsWith('WORKING')?Math.sin(Date.now()/120)*2:0;cx.fillStyle='rgba(0,0,0,.28)';cx.beginPath();cx.ellipse(w.x*T+32,w.y*T+57,21,5,0,0,Math.PI*2);cx.fill();cx.drawImage(im,sx,sy,sw,sh,w.x*T+8,w.y*T-22+bob,48,72);cx.font='13px VT323, monospace';cx.textAlign='center';cx.strokeStyle='#000';cx.lineWidth=3;cx.strokeText(w.candidate.name,w.x*T+32,w.y*T-25);cx.fillStyle='#fff';cx.fillText(w.candidate.name,w.x*T+32,w.y*T-25);}}
function updateWorkers(){if(!map)return;const now=Date.now();for(const w of workers){if(now<w.nextMove)continue;w.nextMove=now+700+Math.random()*900;let target=null;if(w.assignment)target=map.placements.find(p=>p.sprite===w.assignment);if(target){const tx=Math.max(1,target.tile[0]-1),ty=target.tile[1]+target.footprint[1]-1,d=Math.abs(tx-w.x)+Math.abs(ty-w.y),fit=workerSkillFit(w);w.skillFit=fit;if(d<=1){w.workPulse=(w.workPulse||0)+fit;w.status='WORKING · '+(STATION_NAMES[w.assignment]||w.assignment.toUpperCase())+' · SKILL '+fit+'/5';continue}w.status='TRAVELING · '+(STATION_NAMES[w.assignment]||w.assignment.toUpperCase())+' · SKILL '+fit+'/5';const dx=Math.sign(tx-w.x),dy=Math.sign(ty-w.y);if(dx&&!blocked(w.x+dx,w.y))w.x+=dx;else if(dy&&!blocked(w.x,w.y+dy))w.y+=dy}else{w.status=w.assignment?'WAITING FOR '+(STATION_NAMES[w.assignment]||w.assignment.toUpperCase())+' IN THIS FACILITY':'MEANDERING';const dirs=[[1,0],[-1,0],[0,1],[0,-1],[0,0]],q=dirs[Math.floor(Math.random()*dirs.length)];if(!blocked(w.x+q[0],w.y+q[1])){w.x+=q[0];w.y+=q[1]}}}const assigned=workers.filter(w=>w.assignment);cv.dataset.workerCount=workers.length;cv.dataset.assignedWorkers=assigned.length;cv.dataset.averageWorkerSkill=assigned.length?(assigned.reduce((sum,w)=>sum+workerSkillFit(w),0)/assigned.length).toFixed(1):'0.0';}
...
const STATION_NAMES={planning_desk:"PLANNING DESK",tool_cart:"TOOL CART",presetter_t4:"TOOL PRESETTER",toolcrib_rfid_t4:"RFID TOOL CRIB",vmc_t2:"VMC",nox_terminal:"NOX MATERIAL TERMINAL",lathe_cnc_t2:"CNC LATHE",saw_t1:"BANDSAW",mill_manual_t1:"MANUAL MILL",bench_deburr_t1:"DEBURR BENCH",network_node_t3:"MTCONNECT NODE",handoff_terminal_t4:"HANDOFF TERMINAL",nox_pallet:"RECEIVING",whiteboard:"MISSION BOARD",chalkboard:"SHOP CLASS",cobot_t5:"COBOT",amr_t5:"AMR",restroom_station:"EMPLOYEE RESTROOM"};
function stationDefinition(sprite){return STATION_OPERATIONS.stations[sprite]||{name:STATION_NAMES[sprite]||sprite.replaceAll('_',' '),mode:'orientation',cycleMinutes:0,mission:'Review this station and its place in the manufacturing route.',details:'Station details are being prepared.'};}
function stationDurationMs(sprite){const definition=stationDefinition(sprite);return Math.max(1000,Number(definition.gameplaySeconds||definition.cycleMinutes*60)*1000);}
function queueableStation(sprite){return STATION_OPERATIONS.timeModel.queueableStations.includes(sprite)&&stationDefinition(sprite).mode==='playable';}
function installedStationCapacity(sprite){const equipment=equipmentState();return sprite==='vmc_t2'?Math.max(1,equipment.vmc||1):sprite==='lathe_cnc_t2'?Math.max(1,equipment.lathe||1):sprite==='saw_t1'?Math.max(1,equipment.saw||1):1;}
function assignedWorkersAt(sprite){return workers.filter(worker=>worker.assignment===sprite&&worker.candidate?.qualifications?.includes(sprite));}
function processWorkerQueues(){let changed=false;const now=Date.now();for(const queue of Object.values(workerQueues()))for(const item of queue){const next=now>=item.endAt?'complete':now>=item.startedAt?'running':'queued';if(item.status!==next){item.status=next;changed=true;}}if(changed&&gameStarted)saveGame('WORKER QUEUE UPDATE');return changed;}
function queueWorkerTask(sprite){
  processWorkerQueues();const definition=stationDefinition(sprite),assigned=assignedWorkersAt(sprite);
  if(!queueableStation(sprite)){say('This station is not available for queued production.','Use its active management, facility, or orientation workflow.');return false;}
  if(!assigned.length){say('Assign a qualified worker before queuing unattended work.','Open BUILD YOUR TEAM, hire for '+definition.name+', and assign that employee to this station.');return false;}
  if(!state.job||!state.materialOrders.length){say('This station needs a real customer work order.','Accept a customer contract and order its material before assigning unattended production.');return false;}
  const wip=state.workerWip||(state.workerWip={sawBlanks:0,finishedParts:0});
  if(sprite==='vmc_t2'&&(!state.toolReady||state.toolsSet.length<3||wip.sawBlanks<1)){say('The worker needs a prepared blank and verified tool kit.','Complete a bandsaw batch, collect its blank, and prove the three-tool kit before queuing the VMC.');return false;}
  if(sprite==='lathe_cnc_t2'&&wip.sawBlanks<1){say('The lathe needs a controlled prepared blank.','Complete and collect a bandsaw batch before routing a qualified worker to the CNC lathe.');return false;}
  const orderQuantity=Math.max(1,Number(state.job.contract?.quantity||1)),committedForJob=Object.values(workerQueues()).flat().filter(item=>item.jobId===state.job.id).length+wip.sawBlanks+wip.finishedParts;
  if(sprite==='saw_t1'&&committedForJob>=orderQuantity){say('Every unit on this work order is already routed.','Collect or machine the existing work before releasing another blank.');return false;}
  const queue=stationWorkerQueue(sprite),slots=installedStationCapacity(sprite),limit=slots*3,cost=25+definition.cycleMinutes*3;
  if(queue.length>=limit){say(definition.name+' queue is full.','Collect finished work or add qualified workers/equipment capacity. '+queue.length+' of '+limit+' slots are committed.');return false;}
  if(coins<cost){say('Not enough operating cash for this batch.','Material and consumables require '+cost+' coins.');return false;}
  const playerRun=sprite==='vmc_t2'?state.machineRun:stationRuns()[sprite];let chosenSlot=0,earliest=Infinity;
  for(let slot=0;slot<slots;slot++){const last=queue.filter(item=>item.slot===slot).sort((a,b)=>b.endAt-a.endAt)[0],playerAvailable=slot===0&&playerRun?playerRun.endAt:Date.now(),available=Math.max(playerAvailable,last?last.endAt:Date.now());if(available<earliest){earliest=available;chosenSlot=slot;}}
  const startedAt=Math.max(Date.now(),earliest),durationMs=stationDurationMs(sprite),endAt=startedAt+durationMs,worker=assigned[chosenSlot%assigned.length],reward=sprite==='vmc_t2'||sprite==='lathe_cnc_t2'?50+definition.cycleMinutes*8:0;if(sprite==='vmc_t2'||sprite==='lathe_cnc_t2')wip.sawBlanks--;const operation=sprite==='saw_t1'?'CUT BLANK':sprite==='lathe_cnc_t2'?'TURN PART':'MACHINE PART';coins-=cost;addCoins(0);queue.push({id:sprite+'_'+Date.now()+'_'+queue.length,station:sprite,operation,jobId:state.job.id,materialSku:state.materialOrders[0]?.sku||'ORDERED MATERIAL',status:startedAt>Date.now()?'queued':'running',startedAt,endAt,durationMs,slot:chosenSlot,workerId:worker.id,reward,cost});saveGame('WORKER TASK QUEUED');renderProductionHud();say(worker.candidate.name+' accepted '+definition.name+' batch '+queue.length+'.',(startedAt>Date.now()?'Queued behind active work. ':'Started now. ')+formatRunTime(durationMs)+' RPG cycle · '+slots+' installed equipment slot'+(slots===1?'':'s')+'. Buy another machine to run more work in parallel.');return true;
}
function collectWorkerTasks(sprite){processWorkerQueues();const queue=stationWorkerQueue(sprite),complete=queue.filter(item=>item.status==='complete');if(!complete.length)return 0;const reward=complete.reduce((sum,item)=>sum+item.reward,0),wip=state.workerWip||(state.workerWip={sawBlanks:0,finishedParts:0});workerQueues()[sprite]=queue.filter(item=>item.status!=='complete');if(sprite==='saw_t1')wip.sawBlanks+=complete.length;else wip.finishedParts+=complete.length;if(reward)addCoins(reward);awardFounderXp(complete.length*8);saveGame('WORKER OUTPUT COLLECTED');renderProductionHud();say(complete.length+' '+stationDefinition(sprite).name+' batch'+(complete.length===1?'':'es')+' collected.',sprite==='saw_t1'?complete.length+' prepared blank'+(complete.length===1?' is':'s are')+' now available for VMC routing.':complete.length+' finished part'+(complete.length===1?' is':'s are')+' entered output inventory · '+reward+' coins earned.');return complete.length;}
function formatRunTime(milliseconds){const seconds=Math.max(0,Math.ceil(milliseconds/1000)),minutes=Math.floor(seconds/60),remainder=seconds%60;return minutes+':'+String(remainder).padStart(2,'0');}
function stationRun(sprite){if(sprite==='vmc_t2'&&state.machineRun)return state.machineRun;if(stationRuns()[sprite])return stationRuns()[sprite];if(!queueableStation(sprite))return null;processWorkerQueues();const queue=stationWorkerQueue(sprite);return queue.find(item=>item.status==='complete')||queue.find(item=>item.status==='running')||queue.find(item=>item.status==='queued')||null;}
function runIsReady(run){return !!run&&(run.status==='ready'||run.status==='inspection'||run.status==='complete');}
function runProgress(run){if(!run)return 0;const duration=Math.max(1,run.durationMs||(run.endAt-run.startedAt));return Math.max(0,Math.min(1,(Date.now()-run.startedAt)/duration));}
function stationRunLabel(sprite){const run=stationRun(sprite);if(!run)return'';if(runIsReady(run))return'READY TO COLLECT';if(run.status==='queued')return'QUEUED · STARTS IN '+formatRunTime(run.startedAt-Date.now());return formatRunTime(run.endAt-Date.now())+' REMAINING';}
function drawStationTimerBadge(p){const run=stationRun(p.sprite);if(!run)return;const ready=runIsReady(run),label=ready?'READY':formatRunTime(run.endAt-Date.now()),center=(p.tile[0]+p.footprint[0]/2)*T,y=p.tile[1]*T-31;width=Math.max(82,label.length*9+16);cx.save();cx.font='bold 16px VT323, monospace';cx.textAlign='center';cx.fillStyle='rgba(5,10,15,.95)';cx.strokeStyle=ready?'#3fd08a':'#e8b93b';cx.lineWidth=3;cx.fillRect(center-width/2,y-19,width,24);cx.strokeRect(center-width/2,y-19,width,24);cx.fillStyle=ready?'#3fd08a':'#fff';cx.fillText((ready?'✓ ':'◷ ')+label,center,y-2);cx.restore();}
function completeExpiredRuns(){let changed=processWorkerQueues();const saw=stationRuns().saw_t1;if(saw&&saw.status==='running'&&Date.now()>=saw.endAt){saw.status='ready';playSfx("bandsaw_power_off");changed=true;}const vmc=state.machineRun;if(vmc&&vmc.status==='running'&&Date.now()>=vmc.endAt){vmc.status='inspection';machineMotionMode=null;tscene.dataset.motionCode='M30';stopSfxLoop();playSfx("coolant_off");setTimeout(()=>playSfx("spindle_stop"),300);changed=true;}if(changed&&gameStarted)saveGame('PRODUCTION COMPLETE');}
function renderProductionHud(){completeExpiredRuns();const root=document.getElementById('productionHud'),sprites=[...new Set(map?.placements.map(placement=>placement.sprite)||[])],active=sprites.map(sprite=>[sprite,stationRun(sprite)]).filter(([,run])=>run);root.innerHTML=active.map(([sprite,run])=>{const ready=runIsReady(run),queued=run.status==='queued',pct=Math.round(runProgress(run)*100),queueCount=stationWorkerQueue(sprite).length,id=sprite==='vmc_t2'&&state.machineRun===run?'machineFlag':sprite==='saw_t1'&&stationRuns().saw_t1===run?'sawFlag':'workerFlag-'+sprite;return '<button class="stationTimerFlag '+(ready?'ready':'')+'" id="'+id+'" data-run-station="'+sprite+'"><b>'+(STATION_NAMES[sprite]||sprite)+(queueCount?' · '+queueCount+' TASK'+(queueCount===1?'':'S'):'')+'</b><br>'+(ready?'✓ COMPLETE · OPEN STATION':queued?'QUEUED · '+formatRunTime(run.startedAt-Date.now())+' TO START':formatRunTime(run.endAt-Date.now())+' REMAINING')+'<span class="timerTrack"><i style="width:'+pct+'%"></i></span></button>';}).join('');root.querySelectorAll('[data-run-station]').forEach(button=>button.onclick=()=>walkToStation(button.dataset.runStation,true));const activeTimer=document.getElementById('activeStationTimer'),activeBar=document.querySelector('#activeStationProgress i');if(activeTimer&&task.dataset.station){const run=stationRun(task.dataset.station);if(run){activeTimer.textContent=stationRunLabel(task.dataset.station);if(activeBar)activeBar.style.width=Math.round(runProgress(run)*100)+'%';}}cv.dataset.activeStationRuns=active.length;cv.dataset.readyStationRuns=active.filter(([,run])=>runIsReady(run)).length;}
function startStationRun(sprite,durationMs,payload={}){const now=Date.now(),run={station:sprite,status:'running',startedAt:now,endAt:now+durationMs,durationMs,...payload};if(sprite==='vmc_t2')state.machineRun=run;else stationRuns()[sprite]=run;renderProductionHud();saveGame((STATION_NAMES[sprite]||sprite)+' STARTED');return run;}
...
function currentObjective(){
  ...
  if(!state.pendingContract&&!state.job&&!state.contracts.some(contract=>contract.status!=="declined"))return{action:"customers",step:"1. Answer the shop phone or JobLine email",how:"Review the caller, company, quantity, material, due date, and requirements. Accept one contract to create a queued work order."};
  ...
  const sawRun=stationRuns().saw_t1;if(sawRun)return{sprite:"saw_t1",step:sawRun.status==="running"?countLabel+": Bandsaw is cutting the blank":countLabel+": Bandsaw blank is ready",how:sawRun.status==="running"?"Work elsewhere while the accelerated saw timer runs, or open the station to check progress.":"Open the green BANDSAW flag and collect the labeled blank."};
  ...
  return{sprite:"vmc_t2",step:countLabel+": Run and inspect the CNC part",how:"Open the pulsing VMC. Confirm the physical setup, start the timer, and return to inspect the finished shape."};
}
function updateGuide(){if(!map)return;const o=currentObjective(),near=nearStation(),prompt=document.getElementById("stationPrompt"),action=document.getElementById("objectiveAction");
  ...
  document.querySelector('[data-mission-action="customers"]').classList.toggle("done",state.contracts.some(contract=>contract.status!=="declined"));
  ...
  if(o){document.getElementById("objectiveStep").textContent=o.step;document.getElementById("objectiveHow").textContent=kidExplain(o.how);const atTarget=o.sprite&&near&&near.sprite===o.sprite;action.disabled=false;action.textContent=o.action==="customers"?"ANSWER PHONE / OPEN JOBLINE":atTarget?"OPEN "+(STATION_NAMES[o.sprite]||"STATION"):"GO TO & OPEN "+(STATION_NAMES[o.sprite]||"TARGET");}else{document.getElementById("objectiveStep").textContent="Garage mastery complete — graduate to the Job Shop";document.getElementById("objectiveHow").textContent="Five customer jobs shipped. Review the expansion scene and move your company.";action.disabled=true;action.textContent="CHAPTER COMPLETE";}
  if(near){prompt.classList.add("open");const status=stationRunLabel(near.sprite);document.getElementById("stationPromptText").textContent="USE · "+(STATION_NAMES[near.sprite]||near.label||"STATION")+(status?" · "+status:"")+" · E / SPACE / XBOX A / PHONE ●";}else prompt.classList.remove("open");}
document.getElementById("objectiveAction").onclick=()=>{const o=currentObjective(),near=nearStation();if(o?.action==="customers"){openCustomers("shop_phone");return;}if(o&&(!near||near.sprite!==o.sprite))walkToStation(o.sprite,true);else interact(o?.sprite);};document.getElementById("stationOpen").onclick=()=>interact();
...
function openCustomers(channel="jobline_email"){customerChannel=channel;const panel=document.getElementById("customerPanel");let guide=document.getElementById("customerIntakeGuide");if(!guide){guide=document.createElement("div");guide.id="customerIntakeGuide";guide.className="stationStatusCard";document.getElementById("customerHeader").before(guide);}guide.innerHTML='<b>ZACH · CUSTOMER INTAKE</b><br>1 · ANSWER PHONE OR EMAIL &nbsp; 2 · REVIEW PART, QUANTITY, MATERIAL, DUE DATE &nbsp; 3 · ACCEPT TO JOBLINE QUEUE &nbsp; 4 · RELEASE AT PLANNING';document.getElementById("customers").classList.add("open");playSfx(channel==="shop_phone"?"phone_ring":"terminal_wake");if(!state.pendingContract&&!state.contracts.some(contract=>contract.status!=="declined")){say("A customer is calling your new factory.","Review the request, then accept it to create your first JobLine work order.","zach_first_customer_owner");}renderCustomer();}
function acceptCustomerContract(c){const o=c.offer;state.pendingContract={...o,customerId:c.id,customer:c.company,contact:c.contact};state.contracts.push({offerId:o.id,customerId:c.id,status:"accepted"});document.querySelector('[data-mission-action="customers"]').classList.add("done");playSfx("handoff_confirm");say(c.company+" hired "+companyName+".","JobLine queue: 1 work order waiting. Open PLANNING to release traveler "+o.id+".","zach_first_customer_owner");saveGame("CONTRACT ACCEPTED");renderCustomer();updateGuide();if(!storySeen("first_customer")){document.getElementById("customers").classList.remove("open");setTimeout(()=>showStorySequence("first_customer"),100);}}
...
function followPath(path,openWhenThere=false,run=true,preferredSprite=null){cancelMove();const openTarget=()=>tourActive&&tourPhase===2?renderTourPractice():interact(preferredSprite);if(!path.length){if(openWhenThere)openTarget();return;}let i=0,pace=run?82:145;moveTarget={x:P.x+path.reduce((n,d)=>n+d[0],0),y:P.y+path.reduce((n,d)=>n+d[1],0)};moveTimer=setInterval(()=>{if(i>=path.length){cancelMove();if(openWhenThere)setTimeout(openTarget,100);return;}move(path[i][0],path[i][1],true,run);i++;},pace);}
function walkToStation(sprite,openWhenThere=false){const path=pathToStation(sprite),exists=map.placements.some(p=>p.sprite===sprite);if(openWhenThere&&!exists){say('That station is not installed in this facility.','Open the facility or equipment roadmap to review its unlock requirements.');return;}followPath(path,openWhenThere,true,sprite);}

/* ================= INTERACT -> TASKS ================= */
const STATION_SFX={nox_terminal:"terminal_wake",saw_t1:"bandsaw_vise_clamp",tool_cart:"tool_drawer_open",presetter_t4:"probe_touch",toolcrib_rfid_t4:"rfid_tool_scan",vmc_t2:"machine_vise_clamp",lathe_cnc_t2:"lathe_chuck_clamp",planning_desk:"planning_paperwork",chalkboard:"chalk_marker",mill_manual_t1:"manual_mill_run",bench_deburr_t1:"deburr_tool",network_node_t3:"network_connect",handoff_terminal_t4:"handoff_confirm",nox_pallet:"pallet_delivery",whiteboard:"mission_board_update",amr_t5:"amr_drive",cobot_t5:"cobot_motion",restroom_station:"planning_paperwork"};
function restroomState(){const amenities=state.facilityAmenities||(state.facilityAmenities={});return amenities.restroom||(amenities.restroom={status:'available',step:0,morale:100,lastIncidentJob:0});}
function showRestroomView(cell){document.getElementById('tview').style.display='none';tscene.style.display='none';let view=document.getElementById('restroomTaskView');if(!view){view=document.createElement('div');view.id='restroomTaskView';view.style.cssText='width:min(620px,88vw);height:min(460px,52vh);margin:8px auto;background-repeat:no-repeat;background-color:#080b10;image-rendering:pixelated;border:3px solid var(--gold)';tscene.after(view);}const x=cell%2,y=Math.floor(cell/2);view.style.display='block';view.style.backgroundImage='url('+assetUrl(EQUIPMENT_VIEWS['job-shop-restroom-states-v1'])+')';view.style.backgroundSize='200% 200%';view.style.backgroundPosition=(x*100)+'% '+(y*100)+'%';}
function showEquipmentDamage(equipment){const stateView=EQUIPMENT_DAMAGE_STATES.states.find(entry=>entry.equipment===equipment),atlas=stateView&&EQUIPMENT_DAMAGE_STATES.atlases[stateView.atlas];if(!stateView||!atlas)return;document.getElementById('tview').style.display='none';tscene.style.display='none';hideRestroomView();let view=document.getElementById('equipmentDamageView');if(!view){view=document.createElement('div');view.id='equipmentDamageView';view.style.cssText='width:min(620px,88vw);height:min(460px,52vh);margin:8px auto;background-repeat:no-repeat;background-color:#080b10;image-rendering:pixelated;border:3px solid var(--alert)';tscene.after(view);}const x=stateView.cell%atlas.columns,y=Math.floor(stateView.cell/atlas.columns),px=atlas.columns===1?0:x*100/(atlas.columns-1),py=atlas.rows===1?0:y*100/(atlas.rows-1);view.style.display='block';view.style.backgroundImage='url('+assetUrl(EQUIPMENT_VIEWS[atlas.asset])+')';view.style.backgroundSize=(atlas.columns*100)+'% '+(atlas.rows*100)+'%';view.style.backgroundPosition=px+'% '+py+'%';view.setAttribute('aria-label',equipment.replaceAll('_',' ')+' damaged state: '+stateView.fault);}
function hideEquipmentDamage(){const view=document.getElementById('equipmentDamageView');if(view)view.style.display='none';}
function hideRestroomView(){const view=document.getElementById('restroomTaskView');if(view)view.style.display='none';}
function renderWorkerQueuePanel(sprite){processWorkerQueues();const definition=stationDefinition(sprite),assigned=assignedWorkersAt(sprite),queue=stationWorkerQueue(sprite),capacity=installedStationCapacity(sprite),limit=capacity*3,complete=queue.filter(item=>item.status==='complete'),cost=25+definition.cycleMinutes*3;tctrl.innerHTML='<div class="ttl">WORKER PRODUCTION QUEUE · '+definition.name.toUpperCase()+'</div><div class="stationStatusCard"><b>STANDARD CYCLE:</b> '+definition.cycleMinutes+':00 · <b>INSTALLED CAPACITY:</b> '+capacity+' · <b>QUEUE:</b> '+queue.length+' / '+limit+'<br><b>ASSIGNED TEAM:</b> '+(assigned.length?assigned.map(worker=>worker.candidate.name+' · SKILL '+workerSkillFit(worker,sprite)+'/5').join(', '):'NONE — ASSIGN A QUALIFIED WORKER FIRST')+'</div><div class="workerQueueList">'+(queue.length?queue.map((item,index)=>{const worker=workers.find(entry=>entry.id===item.workerId),status=item.status==='complete'?'✓ COMPLETE':item.status==='queued'?'QUEUED · STARTS '+formatRunTime(item.startedAt-Date.now()):formatRunTime(item.endAt-Date.now())+' REMAINING';return '<div class="stationStatusCard"><b>BATCH '+(index+1)+' · '+status+'</b><br>SLOT '+(item.slot+1)+' · '+(worker?.candidate.name||'ASSIGNED WORKER')+' · REWARD '+item.reward+'</div>';}).join(''):'<div class="stationStatusCard">NO BATCHES QUEUED</div>')+'</div><button id="queueWorkerBatch" class="grn" '+(!assigned.length||queue.length>=limit||coins<cost?'disabled':'')+'>QUEUE WORKER BATCH · '+cost+' COINS</button><button id="collectWorkerBatches" '+(complete.length?'class="grn"':'')+' '+(complete.length?'':'disabled')+'>COLLECT '+complete.length+' COMPLETE BATCH'+(complete.length===1?'':'ES')+'</button><button id="workerQueueClose">RETURN TO FACTORY</button>';const queueButton=document.getElementById('queueWorkerBatch');queueButton.onclick=()=>{if(queueWorkerTask(sprite))renderWorkerQueuePanel(sprite);};document.getElementById('collectWorkerBatches').onclick=()=>{collectWorkerTasks(sprite);renderWorkerQueuePanel(sprite);};document.getElementById('workerQueueClose').onclick=closeOverlay;}
function openWorkerQueue(sprite){const definition=stationDefinition(sprite);openOverlay(definition.name.toUpperCase()+' · WORKER QUEUE','Assign qualified employees, load up to three batches per installed station, and let absolute timers continue while you are away.');task.dataset.station=sprite;showStationView(sprite);tz('ZACH: Queue only work your assigned team is qualified to run. Each equipment slot handles one batch at a time; completed output waits for your review and collection.');renderWorkerQueuePanel(sprite);}
function appendWorkerQueueLauncher(sprite){if(!queueableStation(sprite)||!document.getElementById('tcontrols'))return;const button=document.createElement('button');button.className='workerQueueLauncher';button.textContent='WORKER QUEUE · '+stationWorkerQueue(sprite).length+' / '+(installedStationCapacity(sprite)*3)+' BATCHES';button.onclick=()=>openWorkerQueue(sprite);tctrl.append(button);}
function maybeTriggerAmenityEvent(){const amenity=restroomState();if(state.jobsShipped<8||amenity.status!=='available'||state.jobsShipped-amenity.lastIncidentJob<20)return false;if(state.jobsShipped===12||Math.random()<FACILITY_AMENITIES.events[0].rarity.chancePerEligibleShift){amenity.status='service_required';amenity.step=0;amenity.morale=Math.max(0,amenity.morale-2);amenity.lastIncidentJob=state.jobsShipped;saveGame('FACILITY SERVICE EVENT');return true;}return false;}
const STATION_EQUIPMENT_VIEWS={tool_cart:'tool-cart-open-v1',vmc_t2:'vmc-open-v1',lathe_cnc_t2:'lathe-open-v1'};
function showStationView(sprite){const equipmentView=STATION_EQUIPMENT_VIEWS[sprite];if(equipmentView&&EQUIPMENT_VIEWS[equipmentView]){showEquipmentView(equipmentView);return;}hideEquipmentDamage();hideRestroomView();const im=document.getElementById('tview');im.src=assetUrl(SPRITES[sprite]||SPRITES.whiteboard);im.style.display='block';im.style.objectFit='contain';im.style.imageRendering='pixelated';tscene.style.display='none';}
function openStationDetails(p){const definition=stationDefinition(p.sprite),stop=SHOP_TOUR.stops.find(item=>item.sprite===p.sprite),walk=stop&&STATION_WALKTHROUGHS[stop.id],objective=currentObjective(),isObjective=objective?.sprite===p.sprite,run=stationRun(p.sprite),modeLabel=definition.mode==='orientation'?'ORIENTATION ONLY · PRODUCTION MECHANICS NOT YET ACTIVE':definition.mode==='playable'?'PLAYABLE PRODUCTION STATION':definition.mode==='management'?'ACTIVE MANAGEMENT STATION':'ACTIVE FACILITY STATION';openOverlay(definition.name.toUpperCase()+' · STATION DETAILS',modeLabel);task.dataset.station=p.sprite;showStationView(p.sprite);if(walk?.voice)playZach(walk.voice);tz('ZACH: '+(walk?.text||definition.details));const timer=definition.cycleMinutes?definition.cycleMinutes+' MIN STANDARD CYCLE':'NO PRODUCTION TIMER';tctrl.innerHTML='<div class="stationDetailGrid"><div class="stationStatusCard"><span class="stationMode">'+modeLabel+'</span><div class="ttl">STANDARD TIME · '+timer+'</div><p>'+definition.details+'</p></div><div><div class="ttl">STATION MISSION</div><p class="stationMission">'+definition.mission+'</p><div class="stationStatusCard"><b>CURRENT STATUS:</b> '+(run?stationRunLabel(p.sprite):isObjective?'CURRENT OBJECTIVE':'AVAILABLE FOR REVIEW')+'</div>'+(run?'<div id="activeStationTimer" class="ttl">'+stationRunLabel(p.sprite)+'</div><div id="activeStationProgress" class="stationProgress"><i style="width:'+Math.round(runProgress(run)*100)+'%"></i></div>':'')+'<button id="stationDetailsClose">RETURN TO FACTORY</button>'+(objective?'<button id="stationGoObjective" class="grn">GO TO CURRENT MISSION · '+(STATION_NAMES[objective.sprite]||objective.sprite)+'</button>':'')+(p.sprite==='handoff_terminal_t4'?'<button id="stationOpenJobline">OPEN JOBLINE CUSTOMER WORK</button>':'')+'</div></div>';document.getElementById('stationDetailsClose').onclick=closeOverlay;const go=document.getElementById('stationGoObjective');if(go)go.onclick=()=>{closeOverlay();walkToStation(objective.sprite,true);};const jobline=document.getElementById('stationOpenJobline');if(jobline)jobline.onclick=()=>{closeOverlay();openCustomers('jobline_email');};cv.dataset.lastStationDetails=p.sprite;}
function openRestroomTask(){const amenity=restroomState(),event=FACILITY_AMENITIES.events[0],tech=maintenanceWorker('restroom_station');hideRestroomView();if(amenity.status==='available'){openOverlay('EMPLOYEE RESTROOM · AVAILABLE','A clean, working shared space supports the whole team.');showRestroomView(1);tz('ZACH: A good manufacturer maintains the whole workplace, not only the machines. Inspect utilities and shared spaces before they become interruptions.');tctrl.innerHTML='<div class="ttl">STATUS · CLEAN, STOCKED, AVAILABLE</div><button id="restroomInspect" class="grn">COMPLETE FACILITY INSPECTION</button>'+(state.jobsShipped>=5?'<button id="reportRestroomIssue">REPORT A SERVICE ISSUE · TRAINING</button>':'');document.getElementById('restroomInspect').onclick=()=>{saveGame('RESTROOM INSPECTION');closeOverlay();say('Facility inspection recorded.','Restroom is available and the team morale check is '+amenity.morale+'/100.');};const report=document.getElementById('reportRestroomIssue');if(report)report.onclick=()=>{amenity.status='service_required';amenity.step=0;amenity.morale=Math.max(0,amenity.morale-2);saveGame('RESTROOM ISSUE REPORTED');openRestroomTask();};return;}
  openOverlay('RESTROOM SERVICE INTERRUPTION','Protect people, assign qualified help, and verify the facility before reopening.');showRestroomView(2);const steps=event.responseSequence,current=steps[Math.min(amenity.step,steps.length-1)];tz('ZACH: '+event.zachTeachingIntent);const qualified=!!tech,atClear=current.action==='clear',canPay=coins>=event.consequences.resolvedCorrectly.cashCost;tctrl.innerHTML='<div class="ttl">STEP '+(amenity.step+1)+' OF '+steps.length+' · '+current.action.replaceAll('_',' ').toUpperCase()+'</div><p>'+current.instruction+'</p><div class="hint">MORALE '+amenity.morale+'/100 · SERVICE COST '+event.consequences.resolvedCorrectly.cashCost+' COINS</div>'+(atClear&&!qualified?'<button id="hireFacilityTech">HIRE QUALIFIED FACILITIES HELP</button>':'<button id="advanceFacilityRepair" class="grn" '+(atClear&&!canPay?'disabled':'')+'>'+(atClear?'AUTHORIZE '+tech?.candidate.name.toUpperCase()+' · '+event.consequences.resolvedCorrectly.cashCost+' COINS':'COMPLETE THIS STEP')+'</button>');const hire=document.getElementById('hireFacilityTech');if(hire)hire.onclick=()=>{closeOverlay();hideRestroomView();hireIndex=HIRE_ROSTER.candidates.findIndex(candidate=>candidate.maintenanceQualified&&candidate.qualifications.includes('restroom_station'));renderHires();document.getElementById('hire').classList.add('open');};const advance=document.getElementById('advanceFacilityRepair');if(advance)advance.onclick=()=>{if(atClear){coins-=event.consequences.resolvedCorrectly.cashCost;addCoins(0);}amenity.step++;if(amenity.step>=steps.length){amenity.status='available';amenity.step=0;amenity.morale=Math.min(100,amenity.morale+event.consequences.resolvedCorrectly.morale);showRestroomView(3);tz('ZACH: Service restored. The qualified worker sanitized, inspected, documented, and reopened the space.');tctrl.innerHTML='<button id="facilityReturn" class="grn">RETURN TO FACTORY</button>';document.getElementById('facilityReturn').onclick=()=>{hideRestroomView();closeOverlay();};saveGame('FACILITY AMENITY RESTORED');}else{saveGame('FACILITY REPAIR STEP');openRestroomTask();}};}
function interact(preferredSprite=null){
  const p=preferredSprite?map.placements.find(item=>item.sprite===preferredSprite&&Math.max(item.tile[0]-P.x,0,P.x-(item.tile[0]+item.footprint[0]-1))+Math.max(item.tile[1]-P.y,0,P.y-(item.tile[1]+item.footprint[1]-1))<=1):nearStation();
  if(!p){const worker=nearWorker();if(worker){talkWorker(worker);return;}cv.dataset.interaction="too-far";say("Move beside a station before using it.","Follow the pulsing objective or click a machine to walk into interaction range.");return;}
  ...
  if(p.sprite==="nox_terminal"){trackAnalytics('station_start',{station_id:'nox_terminal'});openNoxOrder();return;}
  if(p.sprite==="restroom_station"){trackAnalytics('station_start',{station_id:'restroom_station'});openRestroomTask();return;}
  if(p.sprite==="saw_t1"){if(!state.job){say("No traveler, no cut length.","Accept a customer job at planning first.");return;}trackAnalytics('station_start',{station_id:'saw_t1'});openSawTask();appendWorkerQueueLauncher('saw_t1');return;}
  if(p.sprite==="tool_cart"||p.sprite==="presetter_t4"){trackAnalytics('station_start',{station_id:analyticsId(p.sprite)});openToolTask();appendWorkerQueueLauncher(p.sprite);return;}
  if(p.sprite==="vmc_t2"){
    if(!state.rawStockReady){say("The founder-run vise is empty — raw stock has not been cut.","Cut the blank at the BANDSAW, or manage qualified employee batches in the worker queue.");openWorkerQueue('vmc_t2');return;}
    if(!state.toolReady||state.toolsSet.length<3){say("The founder-run setup calls for three gauged tools.","Finish the tool kit, or manage qualified repeat work in the worker queue.");openWorkerQueue('vmc_t2');return;}
    trackAnalytics('station_start',{station_id:'vmc_t2'});openVmcTask();appendWorkerQueueLauncher('vmc_t2');return;}
  if(p.sprite==="lathe_cnc_t2"){trackAnalytics('station_start',{station_id:'lathe_cnc_t2'});setTimeout(()=>playSfx("lathe_turret_index"),700);openWorkerQueue(p.sprite);return;}
  if(p.sprite==="planning_desk"){const contract=state.pendingContract;state.job=contract?JOBS.find(j=>j.id===contract.jobFamily):JOBS[jobIdx%JOBS.length];state.rawStockReady=false;state.toolReady=false;state.toolsSet=[];if(contract){state.job={...state.job,contract};state.pendingContract=null;const r=state.contracts.find(x=>x.offerId===contract.id);if(r)r.status="released to production";}
    trackAnalytics('station_start',{station_id:'planning_desk'});say(contract?contract.customer+" work order released: "+state.job.card:"Job card: "+state.job.card,contract?"JobLine traveler created for "+contract.quantity+" pieces. First operation: cut the raw blank.":"First operation: cut the raw blank at the bandsaw.");mission("planning_desk");updateGuide();return;}
  trackAnalytics('station_start',{station_id:analyticsId(p.sprite)});if(p.sprite==='chalkboard')mission('chalkboard');openStationDetails(p);appendWorkerQueueLauncher(p.sprite);
}
...
function contextTaskTutorialId(){if(task.classList.contains("open")&&activeTaskTutorialId)return activeTaskTutorialId;if(!state.materialOrders.length)return"order_material";if(!state.job)return"accept_job";if(!state.rawStockReady)return"cut_raw_stock";if(!state.tool)return"select_primary_tool";if(!state.stickout)return"set_stickout";if(!state.toolReady)return"complete_tool_kit";return"prepare_cnc_run";}
...
function closeOverlay(){task.classList.remove("open");delete task.dataset.station;delete tscene.dataset.liveProduction;delete tscene.dataset.partShape;delete tscene.dataset.productionProgress;hideRestroomView();hideEquipmentDamage();cancelAnimationFrame(taskAnim);taskAnim=null;clearCycleTimers();stopSfxLoop();machineMotionMode=null;tctrl.innerHTML="";updateGuide();}
function showEquipmentView(id){if(id==='vmc-open-v1'&&maintenanceState().condition<=20){showEquipmentDamage('vmc');return;}if(id==='vmc-open-v1'&&state.machineRun?.status==='running'){showTaskCanvas();watchMachineRun(state.job||JOBS.find(job=>job.id===state.machineRun.jobId),state.machineRun);return;}hideEquipmentDamage();const im=document.getElementById("tview");im.src=assetUrl(EQUIPMENT_VIEWS[id]);im.style.display="block";tscene.style.display="none";}
...
const ONBOARDING_PRACTICE={
 ...
 tour_vmc:[{prompt:"Which sequence is safe before Cycle Start?",options:["Run first, then inspect","Blank → workholding → tools → setup check","Open the door and start the spindle"],correct:1,success:"The setup is confirmed before motion begins."},{prompt:"What can you do after the timed VMC simulation starts?",options:["Return to the floor and watch its timer flag","Load another part into the same busy machine","Turn off saving"],correct:0,success:"The station keeps working while you manage the rest of the shop."}],
...
function openNoxOrder(){
  ...
  task.dataset.station="nox_terminal";
...
function placeNoxOrder(item){
  ...
  saveGame('MATERIAL ORDER');trackAnalytics('task_complete',{task_id:'material_order',station_id:'nox_terminal'});
...
/* ================= RAW STOCK: BANDSAW ================= */
function openSawTask(){const J=state.job;completeExpiredRuns();const active=stationRuns().saw_t1;if(active){openOverlay("BANDSAW — "+(runIsReady(active)?"BLANK READY":"CUT IN PROGRESS"),J.card);task.dataset.station='saw_t1';showTaskCanvas();drawSawCut(active.cutLength,J.stockLength,runIsReady(active));tz(runIsReady(active)?"ZACH: The saw cycle is complete. Verify the blank identity and collect it for the CNC setup.":"ZACH: The saw is cutting autonomously. Keep clear and use this time for tooling, planning, or customer work.");tctrl.innerHTML='<div id="activeStationTimer" class="ttl">'+stationRunLabel('saw_t1')+'</div><div id="activeStationProgress" class="stationProgress"><i style="width:'+Math.round(runProgress(active)*100)+'%"></i></div><div class="stationStatusCard">STANDARD CYCLE · 5:00 · CUT LENGTH '+Number(active.cutLength).toFixed(2)+' IN</div>'+(runIsReady(active)?'<button id="collectSawBlank" class="grn">COLLECT, LABEL & ROUTE BLANK</button>':'<button id="leaveSawRunning" class="grn">RETURN TO FLOOR · KEEP CUTTING</button>');const collect=document.getElementById('collectSawBlank');if(collect)collect.onclick=()=>{state.rawStockReady=true;delete stationRuns().saw_t1;trackAnalytics('task_complete',{task_id:'stock_cut',station_id:'saw_t1'});saveGame('STOCK COLLECTED');closeOverlay();say('Raw blank collected at '+Number(active.cutLength).toFixed(2)+' inches.','Next: gauge the full CNC tool kit.');};const leave=document.getElementById('leaveSawRunning');if(leave)leave.onclick=closeOverlay;return;}openOverlay("BANDSAW — CUT RAW STOCK",J.card);task.dataset.station='saw_t1';showTaskCanvas();playSfx("bandsaw_power_on");curHints=["Saw blanks need finish allowance.","Set the stop to the traveler length, not the final print length.","The gold line is the required saw length: "+J.stockLength.toFixed(2)+' inches.'];
playZach("zach_cut_stock");
  ...
  document.getElementById("cutStock").onclick=()=>{const v=Number(sl.value);if(Math.abs(v-J.stockLength)>.08){attempts++;tz("ZACH: Almost—adjust the stop toward "+J.stockLength.toFixed(2)+" inches. Try again; mistakes cost nothing here.");return;}playSfx("bandsaw_cut");const duration=stationDurationMs('saw_t1');startStationRun('saw_t1',duration,{jobId:J.id,cutLength:v});drawSawCut(v,J.stockLength);closeOverlay();say("Bandsaw cycle started · "+formatRunTime(duration)+".","The real standard is 5:00; this RPG cycle is accelerated. Start another useful task, then return to the green completion flag.");};}
...
function openToolTask(){
  ...
  const J=state.job;
  openOverlay("TOOL SETUP — "+(J.gtitle),J.card);task.dataset.station="tool_cart";showEquipmentView("tool-cart-open-v1");
  curHints=J.hints.tool;
  tz("ZACH: Read the job card. Which tool does this work want? Pick one.");
  // step 1: pick tool
  tctrl.innerHTML=partPreview(J);
...
function stepToolKit(){showEquipmentView("tool-cart-open-v1");curHints=["The probe establishes where the stock is.","The chamfer tool breaks sharp edges before inspection.","A complete kit is primary cutter, probe, and chamfer tool."];tz("ZACH: Gauge the probe and chamfer tool before loading the machine.");
  ...
  document.getElementById("installKit").onclick=()=>{state.toolReady=true;mission("task_tool");saveGame('TOOL KIT');trackAnalytics('task_complete',{task_id:'tool_setup',station_id:'tool_cart'});closeOverlay();say("Three tools gauged and installed: cutter, probe, chamfer.","Stock and tool kit are ready. Open the VMC, confirm the physical setup, and start the timed simulation.");};}

/* ================= TASK 2: VMC G/M + RUN ================= */
function machineFlag(){renderProductionHud();return document.getElementById('machineFlag');}
function refreshMachineRun(){renderProductionHud();cv.dataset.machineStatus=state.machineRun?.status||'idle';}
function startAutonomousRun(J,fromProof=false){if(state.machineRun){say("The VMC cell is occupied.","Wait for the timer and inspect the part before loading more work. Purchase another VMC to add a separate parallel production slot.");return;}const capacity=Math.max(1,equipmentState().vmc||1),teamTime=Math.max(-.35,assignedTeamEffect('proveOutTime','vmc_t2')+assignedTeamEffect('setupTime','vmc_t2')),baseDuration=stationDurationMs('vmc_t2'),duration=Math.max(20000,Math.round(baseDuration*(1+teamTime)));startStationRun('vmc_t2',duration,{jobId:J.id,teamTimeBonus:teamTime,capacity});if(!fromProof){playSfx("cnc_door_close");setTimeout(()=>playSfx("spindle_start"),500);}setTimeout(()=>playSfx(J.tool==="twist"?"drill_cut_aluminum":J.tool==="ball"?"ball_mill_cut_aluminum":"end_mill_cut_aluminum",{loop:true}),fromProof?0:1400);closeOverlay();say("Verified VMC production lot running · "+formatRunTime(duration)+".","The real standard is 10:00; this RPG cycle is accelerated. Buy more machines to add parallel production slots and grow earnings.");refreshMachineRun();}
setInterval(refreshMachineRun,500);
function openVmcTask(){
  completeExpiredRuns();
  task.dataset.station="vmc_t2";
  const condition=maintenanceState();if(condition.condition<=20){openOverlay("VMC — DOWN FOR MAINTENANCE","Condition "+condition.condition+"% · production lockout protects the machine and the part.");showEquipmentView("vmc-open-v1");const tech=maintenanceWorker('vmc_t2');tz(tech?"ZACH: "+tech.candidate.name+" is qualified to diagnose, service, test, and release this machine. Authorize the repair before production.":"ZACH: This machine is down. Hire a maintenance technician qualified on the VMC before anyone attempts a repair or another cycle.");tctrl.innerHTML='<div class="ttl">CONDITION '+condition.condition+'% · STATUS DOWN</div><div class="hint">REPAIR SEQUENCE: LOCKOUT → DIAGNOSE → REPAIR → TEST → RELEASE</div>'+(tech?'<button id="repairVmc" class="grn" '+(coins<350?'disabled':'')+'>AUTHORIZE '+tech.candidate.name.toUpperCase()+' · 350 COINS</button>':'<button id="hireMaintenance">HIRE MAINTENANCE TECHNICIAN</button>');const repair=document.getElementById("repairVmc");if(repair)repair.onclick=()=>{if(serviceVmc()){tz("ZACH: Maintenance complete. The technician tested and released the VMC at 100% condition.");openVmcTask();}};const hire=document.getElementById("hireMaintenance");if(hire)hire.onclick=()=>{closeOverlay();hireIndex=HIRE_ROSTER.candidates.findIndex(candidate=>candidate.maintenanceQualified&&candidate.qualifications.includes('vmc_t2'));renderHires();document.getElementById('hire').classList.add('open');};return;}
  ...
  if(state.machineRun){const run=state.machineRun,J=state.job||JOBS.find(x=>x.id===run.jobId);refreshMachineRun();if(run.status==="inspection"){openOverlay("VMC — CHECK THE FINISHED PART","The machine is done, but the job is not shipped until you check it.");task.dataset.station='vmc_t2';showEquipmentView("vmc-open-v1");tz("ZACH: Compare the finished part with the job picture. Check the shape, holes, and clean edges.");tctrl.innerHTML='<div class="ttl">✓ 10-MINUTE PRODUCTION CYCLE COMPLETE</div><button id="inspectPart" class="grn">COMPARE PART TO JOB PICTURE</button><div id="inspectionStatus" class="hint">PART NOT SHIPPED YET</div>';document.getElementById("inspectPart").onclick=()=>{document.getElementById("inspectionStatus").textContent="✔ SHAPE · HOLES · EDGES MATCH";document.getElementById("inspectPart").textContent="APPROVE CHECK & SHIP PART";document.getElementById("inspectPart").onclick=()=>{state.machineRun=null;showTaskCanvas();finishRun(J);};};return;}openOverlay("VMC — AUTONOMOUS CYCLE",J.card);task.dataset.station='vmc_t2';showEquipmentView("vmc-open-v1");tz("ZACH: The verified cycle is running. Stay clear of the enclosure and review the remaining time.");tctrl.innerHTML='<div id="activeStationTimer" class="ttl">'+stationRunLabel('vmc_t2')+'</div><div id="activeStationProgress" class="stationProgress"><i style="width:'+Math.round(runProgress(run)*100)+'%"></i></div><div class="stationStatusCard">STANDARD CYCLE · 10:00 · VERIFIED G/M CODE</div><button id="leaveRunning" class="grn">RETURN TO FLOOR · KEEP RUNNING</button>';document.getElementById("leaveRunning").onclick=closeOverlay;return;}
  activeTaskTutorialId="prepare_cnc_run";
  playZach("zach_response_safe_stop");
  const J=state.job; curHints=J.hints.g; hintI=0;
  openOverlay("VMC — "+J.gtitle, J.card);showTaskCanvas();drawPartSetup(J);playSfx("machine_power_on",{volume:.7});
  let progHtml='<div class="gcode">';
  for(const ln of J.prog){
    progHtml+=ln.replace(/{{(\w)}}/g,(m,k)=>J.blanks[k].ans[0])+"<br>";
  }
  progHtml+="</div>";
  tz("ZACH: The blank and tool kit are ready. Confirm the setup, then start the timed run. You can leave the station and watch its flag while it works.");
  tctrl.innerHTML=partPreview(J)+'<div class="stationStatusCard"><b>QUICK SETUP · TWO CHECKS, THEN RUN</b><br>Match the part picture, secure the blank, and confirm the loaded tool before the doors close.</div><div class="gline"><button type="button" class="setupTab" data-setup-check="workholding">1 · SECURE STOCK</button><button type="button" class="setupTab" data-setup-check="tool">2 · CONFIRM TOOL</button></div><div class="gline"><button id="toggleSetupView">VIEW VMC EQUIPMENT</button><button id="startProduction" class="grn" disabled>3 · START 10:00 PRODUCTION</button></div><details><summary class="ttl">OPTIONAL SHOP CLASS · VIEW TRUE G & M CODE</summary>'+progHtml+'<div class="hint"><b>G54</b> finds the saved part origin · <b>M03</b> starts the spindle clockwise · <b>M08</b> turns coolant on. This reference does not block the RPG production run.</div></details>';
  const setupChecks=new Set();task.dataset.setupReady="false";
  document.querySelectorAll('[data-setup-check]').forEach(button=>button.onclick=()=>{setupChecks.add(button.dataset.setupCheck);button.classList.add('selected');button.disabled=true;button.textContent='✓ '+button.textContent.slice(4);task.dataset.setupReady=String(setupChecks.size===2);document.getElementById('startProduction').disabled=setupChecks.size!==2;playSfx('terminal_confirm');});
  let showingEquipment=false;document.getElementById("toggleSetupView").onclick=()=>{showingEquipment=!showingEquipment;if(showingEquipment)showEquipmentView("vmc-open-v1");else{showTaskCanvas();drawPartSetup(J);}document.getElementById("toggleSetupView").textContent=showingEquipment?"VIEW PART DRAWING":"VIEW VMC EQUIPMENT";};
  document.getElementById("startProduction").onclick=()=>{if(setupChecks.size!==2)return;tz("ZACH: Setup confirmed. Doors closed. The station will keep working while you return to the factory floor.");startAutonomousRun(J);};
}
...
function drawPartSetup(J){
  tx.fillStyle="#071019";tx.fillRect(0,0,480,260);tx.fillStyle="#e8b93b";tx.font="20px VT323, monospace";tx.fillText("JOB PRINT · "+J.gtitle,14,24);tx.fillStyle="#aeb9c3";tx.font="15px VT323, monospace";tx.fillText("6061-T6 · SAW BLANK "+J.stockLength.toFixed(2)+" IN",14,46);
  if(J.anim==="drill"){tx.fillStyle="#aab4bd";tx.fillRect(82,72,316,130);tx.strokeStyle="#101820";tx.lineWidth=3;tx.strokeRect(82,72,316,130);for(const x of[135,205,275,345]){tx.fillStyle="#071019";tx.beginPath();tx.arc(x,137,11,0,Math.PI*2);tx.fill();tx.strokeStyle="#e8b93b";tx.lineWidth=2;tx.beginPath();tx.moveTo(x-16,137);tx.lineTo(x+16,137);tx.moveTo(x,121);tx.lineTo(x,153);tx.stroke();}tx.fillStyle="#fff";tx.fillText("4X Ø0.257 THRU · 0.55 DEEP",135,229);}
  else if(J.anim==="pocket"){tx.fillStyle="#aab4bd";tx.fillRect(74,64,332,146);tx.strokeStyle="#101820";tx.lineWidth=3;tx.strokeRect(74,64,332,146);tx.fillStyle="#61717d";tx.fillRect(142,91,196,92);tx.strokeStyle="#e8b93b";tx.strokeRect(142,91,196,92);for(let x=148;x<334;x+=12){tx.beginPath();tx.moveTo(x,179);tx.lineTo(Math.min(338,x+75),95);tx.stroke();}tx.fillStyle="#fff";tx.fillText("2.00 × 1.40 POCKET · 0.375 DEEP",105,234);}
  else{tx.fillStyle="#aab4bd";tx.fillRect(70,166,340,48);tx.strokeStyle="#101820";tx.lineWidth=3;tx.strokeRect(70,166,340,48);tx.fillStyle="#73838e";tx.beginPath();tx.moveTo(140,166);tx.quadraticCurveTo(240,40,340,166);tx.closePath();tx.fill();tx.strokeStyle="#e8b93b";tx.lineWidth=3;tx.beginPath();tx.moveTo(140,166);tx.quadraticCurveTo(240,40,340,166);tx.stroke();tx.fillStyle="#fff";tx.fillText("3D DOME · R0.75 FINISH CONTOUR",118,238);}
}
function watchMachineRun(J,run){
  cancelAnimationFrame(taskAnim);showTaskCanvas();tscene.dataset.partShape=J.anim;tscene.dataset.liveProduction="true";
  const frame_=now=>{if(!task.classList.contains('open')||state.machineRun!==run||run.status!=='running')return;const pct=runProgress(run),pulse=(Math.sin(now/90)+1)/2;tscene.dataset.productionProgress=String(Math.round(pct*100));tx.fillStyle='#071019';tx.fillRect(0,0,480,260);tx.fillStyle='#e8b93b';tx.font='17px VT323, monospace';tx.fillText(J.gtitle+' · LIVE CUT VIEW',12,19);tx.fillStyle='#fff';tx.font='14px VT323, monospace';tx.textAlign='right';tx.fillText(Math.round(pct*100)+'% · '+formatRunTime(run.endAt-Date.now())+' LEFT',468,38);tx.textAlign='left';tx.fillStyle='#202a31';tx.fillRect(12,46,456,9);tx.fillStyle='#3fd08a';tx.fillRect(12,46,456*pct,9);
    if(J.anim==='drill'){tx.fillStyle='#aab4bd';tx.fillRect(72,72,336,132);tx.strokeStyle='#101820';tx.lineWidth=3;tx.strokeRect(72,72,336,132);const holes=[125,205,285,355],scaled=pct*holes.length;holes.forEach((x,index)=>{const depth=Math.max(0,Math.min(1,scaled-index));tx.fillStyle='#26333d';tx.beginPath();tx.arc(x,137,11*depth,0,Math.PI*2);tx.fill();tx.strokeStyle=index<Math.ceil(scaled)?'#e8b93b':'#596671';tx.beginPath();tx.arc(x,137,12,0,Math.PI*2);tx.stroke();});const active=Math.min(holes.length-1,Math.floor(scaled));drawHolder(tx,holes[active],62);drawTool(tx,'twist',holes[active],62,.55+.28*pulse,55,8);tx.fillStyle='#fff';tx.fillText('PECK DRILL · 4X Ã˜0.257 · DEPTH 0.55',100,231);}
    else if(J.anim==='pocket'){tx.fillStyle='#aab4bd';tx.fillRect(70,66,340,142);tx.strokeStyle='#101820';tx.lineWidth=3;tx.strokeRect(70,66,340,142);tx.strokeStyle='#e8b93b';tx.strokeRect(142,91,196,92);tx.save();tx.beginPath();tx.rect(142,91,196,92);tx.clip();tx.fillStyle='#667783';tx.fillRect(142,91,196*pct,92);tx.strokeStyle='#34434d';for(let y=100;y<184;y+=12){tx.beginPath();tx.moveTo(142,y);tx.lineTo(142+196*pct,y);tx.stroke();}tx.restore();const pass=(pct*8)%1,row=Math.min(7,Math.floor(pct*8)),x=row%2===0?142+196*pass:338-196*pass,y=98+row*11;tx.fillStyle='#dfe6eb';tx.beginPath();tx.arc(x,y,9,0,Math.PI*2);tx.fill();tx.strokeStyle='#111';tx.stroke();tx.fillStyle='#fff';tx.fillText('2.00 Ã— 1.40 POCKET · FLAT FLOOR RETAINED',87,231);}
    else{const left=110,right=370,base=207,top=72;tx.fillStyle='#aab4bd';tx.beginPath();tx.moveTo(left,base);for(let x=left;x<=right;x+=5){const u=(x-240)/130,target=166-(1-u*u)*94,y=166+(target-166)*pct;tx.lineTo(x,y);}tx.lineTo(right,base);tx.closePath();tx.fill();tx.strokeStyle='#e8b93b';tx.lineWidth=3;tx.beginPath();for(let x=left;x<=right;x+=5){const u=(x-240)/130,target=166-(1-u*u)*94,y=166+(target-166)*pct;x===left?tx.moveTo(x,y):tx.lineTo(x,y);}tx.stroke();const toolX=left+(right-left)*((pct*12)%1),u=(toolX-240)/130,target=166-(1-u*u)*94,toolY=166+(target-166)*pct;drawHolder(tx,toolX,toolY-8);drawTool(tx,'ball',toolX,toolY-8,.55,55,8);tx.fillStyle='#fff';tx.fillText('BALL END MILL · R0.75 DOME · METAL RETAINED',84,231);}
    taskAnim=requestAnimationFrame(frame_);};taskAnim=requestAnimationFrame(frame_);
}
/* animated run scenes */
function runAnim(J){
  showTaskCanvas();
  ...
  function frame_(){
    ...
    if(seg>=pathPts.length-1){ startAutonomousRun(J,true); return; }
...
function finishRun(J){
  ...
  addCoins(pay); mission("task_vmc");state.equipmentCooldownUntil=Date.now()+Math.max(3000,10000-(equipmentState().vmc-1)*2500);applyVmcWear();state.toolReady=false;state.rawStockReady=false;state.toolsSet=[];state.jobsShipped++;maybeTriggerAmenityEvent();state.grades.push({A:5,B:4,C:3,D:2,F:1}[g]);awardFounderXp(60+({A:5,B:4,C:3,D:2,F:1}[g])*10);if(J.contract){const r=state.contracts.find(x=>x.offerId===J.contract.id);if(r)r.status="delivered · grade "+g;state.reputation=Math.max(0,Math.min(100,state.reputation+({A:6,B:4,C:2,D:-1,F:-4}[g])));}jobIdx++; state.job=null;trackAnalytics('task_complete',{task_id:'cnc_cycle',station_id:'vmc_t2'});updateGuide();
...
document.addEventListener("keydown",e=>{
  const target=e.target;if(target instanceof HTMLElement&&(target.matches("input,textarea,select")||target.isContentEditable))return;
...
function pollGamepad(now){
  ...
  if(gp&&(inputMode==="auto"||inputMode==="gamepad")){
    ...
    const preFounderScreen=document.getElementById("preFounder"),founderScreenOpen=preFounderScreen.classList.contains("selecting")&&!preFounderScreen.classList.contains("closed");
    ...
    if(!paused&&edge(0)){if(founderScreenOpen)document.getElementById("newGame").click();else if(!preFounderScreen.classList.contains("closed"))document.getElementById("preFounderNext").click();else if(!task.classList.contains("open"))interact();}
...
html = (html.replace("__SPRITES__", json.dumps(sprites))
            ...
            .replace("__STATION_OPERATIONS__", json.dumps(station_operations))
