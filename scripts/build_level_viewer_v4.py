...
function assignWorker(h,value){const w=workerFor(h.id);if(!w||(!h.qualifications.includes(value)&&value))return;w.assignment=value||null;w.skillFit=workerSkillFit(w);w.status=w.assignment?'TRAVELING TO '+(STATION_NAMES[w.assignment]||w.assignment.toUpperCase())+' · SKILL '+w.skillFit+'/5':'MEANDERING';saveGame('TEAM ASSIGNMENT');renderHires();}
function hireCandidate(h){if(hired.has(h.id))return;if(coins<h.hireCost)return alert("Not enough coins.");const returnToProfile=document.getElementById("profile").classList.contains("open");coins-=h.hireCost;hired.add(h.id);workers.push({id:h.id,candidate:h,x:P.x+1,y:P.y,assignment:null,status:'MEANDERING',skillFit:0,nextMove:0,workPulse:0});addCoins(0);saveGame('TEAM HIRE');renderHires();if(!storySeen("first_hire")){document.getElementById("hire").classList.remove("open");setTimeout(()=>showStorySequence("first_hire",()=>{document.getElementById("hire").classList.add("open");if(returnToProfile)showProfileV2(h);else renderHires();}),100);}}
...
function interact(){
  ...
  if(p.sprite==="restroom_station"){openRestroomTask();return;}
