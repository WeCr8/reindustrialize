...
const state={ toolReady:false, tool:null, stickout:0, toolsSet:[], rawStockReady:false, job:null, materialOrders:[], jobsShipped:0, grades:[],pendingContract:null,contracts:[],storySequences:[],reputation:CUSTOMER_CONTRACTS.reputationRules.starting,founder:{level:1,xp:0,upgradePoints:0,stats:null},machineRun:null,stationRuns:{},workerQueues:{},workerWip:{sawBlanks:0,finishedParts:0},equipment:{vmc:1,saw:1,inspection:1},equipmentCooldownUntil:0 };let paused=false,gameStarted=false,debugOpen=false;
...
function installedStationCapacity(sprite){const equipment=equipmentState();return sprite==='vmc_t2'?Math.max(1,equipment.vmc||1):sprite==='saw_t1'?Math.max(1,equipment.saw||1):1;}
...
function queueWorkerTask(sprite){
  ...
  const startedAt=Math.max(Date.now(),earliest),durationMs=definition.cycleMinutes*60000,endAt=startedAt+durationMs,worker=assigned[chosenSlot%assigned.length],reward=sprite==='vmc_t2'?50+definition.cycleMinutes*8:0;if(sprite==='vmc_t2')wip.sawBlanks--;coins-=cost;addCoins(0);queue.push({id:sprite+'_'+Date.now()+'_'+queue.length,station:sprite,operation:sprite==='saw_t1'?'CUT BLANK':'MACHINE PART',jobId:state.job.id,materialSku:state.materialOrders[0]?.sku||'ORDERED MATERIAL',status:startedAt>Date.now()?'queued':'running',startedAt,endAt,durationMs,slot:chosenSlot,workerId:worker.id,reward,cost});saveGame('WORKER TASK QUEUED');renderProductionHud();say(worker.candidate.name+' accepted '+definition.name+' batch '+queue.length+'.',(startedAt>Date.now()?'Queued behind active work. ':'Started now. ')+formatRunTime(durationMs)+' standard cycle · '+slots+' installed equipment slot'+(slots===1?'':'s')+'.');return true;
}
...
function interact(preferredSprite=null){
  ...
  if(p.sprite==="lathe_cnc_t2"){trackAnalytics('station_start',{station_id:'lathe_cnc_t2'});setTimeout(()=>playSfx("lathe_turret_index"),700);openStationDetails(p);appendWorkerQueueLauncher(p.sprite);return;}
...
playZach("zach_cut_stock");
  ...
  document.getElementById("cutStock").onclick=()=>{const v=Number(sl.value);if(Math.abs(v-J.stockLength)>.08){attempts++;tz("ZACH: That blank misses the traveler length. Reset the stop before cutting.");return;}playSfx("bandsaw_cut");const duration=stationDefinition('saw_t1').cycleMinutes*60000;startStationRun('saw_t1',duration,{jobId:J.id,cutLength:v});drawSawCut(v,J.stockLength);closeOverlay();say("Bandsaw cycle started · 5:00.","The timer continues while you work elsewhere or close the game. Return when the green completion flag appears.");};}
...
function startAutonomousRun(J,fromProof=false){if(state.machineRun){say("The VMC cell is occupied.","Wait for the timer and inspect the part before loading more work. Purchase another VMC to add a separate parallel production slot.");return;}const capacity=Math.max(1,equipmentState().vmc||1),teamTime=Math.max(-.35,assignedTeamEffect('proveOutTime','vmc_t2')+assignedTeamEffect('setupTime','vmc_t2')),baseDuration=stationDefinition('vmc_t2').cycleMinutes*60000,duration=Math.max(240000,Math.round(baseDuration*(1+teamTime)));startStationRun('vmc_t2',duration,{jobId:J.id,teamTimeBonus:teamTime,capacity});if(!fromProof){playSfx("cnc_door_close");setTimeout(()=>playSfx("spindle_start"),500);}setTimeout(()=>playSfx(J.tool==="twist"?"drill_cut_aluminum":J.tool==="ball"?"ball_mill_cut_aluminum":"end_mill_cut_aluminum",{loop:true}),fromProof?0:1400);closeOverlay();say("Verified VMC production lot running · "+formatRunTime(duration)+".","This machine keeps its standard cycle. Additional VMCs add parallel slots for qualified employee work.");refreshMachineRun();}
